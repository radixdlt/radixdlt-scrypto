...
pub fn stop_timed_recovery_with_no_access_fails() {
    ...
    let manifest = ManifestBuilder::new()
        .call_method(
            ...
            "stop_timed_recovery",
            manifest_transcode(&AccessControllerStopTimedRecoveryInput {
                rule_set: RuleSet {
    ...
    pub fn create_proof(&mut self, as_role: Role) -> TransactionReceipt {
        let manifest = self
            ...
            .call_method(
                ...
                "create_proof",
                manifest_transcode(&AccessControllerCreateProofInput {}).unwrap(),
            )
    ...
    pub fn initiate_recovery(
        ...
        let manifest = self
            ...
            .call_method(
                self.access_controller_component_address,
                method_name,
                manifest_transcode(&AccessControllerInitiateRecoveryAsPrimaryInput {
                    rule_set: RuleSet {
    ...
    pub fn timed_confirm_recovery(
        ...
        let manifest = self
            ...
            .call_method(
                ...
                manifest_transcode(&AccessControllerTimedConfirmRecoveryInput {
                    rule_set: RuleSet {
                        primary_role: proposed_primary_role,
                        recovery_role: proposed_recovery_role,
                        confirmation_role: proposed_confirmation_role,
    ...
    pub fn cancel_recovery_attempt(&mut self, as_role: Role) -> TransactionReceipt {
        ...
        let manifest = self
            ...
            .call_method(
                self.access_controller_component_address,
                method_name,
                manifest_transcode(&AccessControllerCancelPrimaryRoleRecoveryProposalInput)
                    .unwrap(),
            )
            .build();
        self.execute_manifest(manifest)
    ...
    pub fn lock_primary_role(&mut self, as_role: Role) -> TransactionReceipt {
        let manifest = self
            ...
            .call_method(
                self.access_controller_component_address,
                "lock_primary_role",
                manifest_transcode(&AccessControllerLockPrimaryRoleInput {}).unwrap(),
            )
            .build();
        self.execute_manifest(manifest)
    }
    ...
    pub fn unlock_primary_role(&mut self, as_role: Role) -> TransactionReceipt {
        let manifest = self
            ...
            .call_method(
                self.access_controller_component_address,
                "unlock_primary_role",
                manifest_transcode(&AccessControllerUnlockPrimaryRoleInput {}).unwrap(),
            )
            .build();
        self.execute_manifest(manifest)
    }

...
use radix_engine_interface::api::{ClientApi, LockFlags, ObjectModuleId, OBJECT_HANDLE_OUTER_OBJECT, OBJECT_HANDLE_SELF, FieldValue};
...
fn cannot_store_reference_in_non_transient_blueprint() {
                ...
                Y: ClientApi<RuntimeError> + KernelNodeApi + KernelSubstateApi<SystemLockData>,
        {
            match export_name {
                "new" => {
                    ...
                    let node_id = api.new_simple_object(BLUEPRINT_NAME, vec![FieldValue::new(Reference(auth_zone))])?;
    ...
    // Assert
    receipt.expect_specific_failure(|e| matches!(e, RuntimeError::SystemError(SystemError::InvalidReference)));
}
...
fn cannot_write_reference_in_non_transient_blueprint() {
    ...
    impl VmInvoke for TestInvoke {
                ...
                Y: ClientApi<RuntimeError> + KernelNodeApi + KernelSubstateApi<SystemLockData>,
        {
            match export_name {
                "new" => {
                    let node_id = api.new_simple_object(BLUEPRINT_NAME, vec![FieldValue::new(())])?;
...
fn cannot_write_reference_in_kv_store() {
    ...
    impl VmInvoke for TestInvoke {
                ...
                Y: ClientApi<RuntimeError> + KernelNodeApi + KernelSubstateApi<SystemLockData>,
        {
            match export_name {
                "kv_store" => {
                    let kv_store = api.key_value_store_new(KeyValueStoreSchema::new::<(), Reference>(false))?;
                    let handle = api.key_value_store_open_entry(&kv_store, &scrypto_encode(&()).unwrap(), LockFlags::MUTABLE)?;
                    let auth_zone = api.get_auth_zone()?;
    ...
    let receipt = test_runner.execute_manifest(
        ManifestBuilder::new()
            .lock_fee(test_runner.faucet_component(), 500u32)
            .call_function(package_address, BLUEPRINT_NAME, "kv_store", manifest_args!())
            .build(),
        vec![],
    );

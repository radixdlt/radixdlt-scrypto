...
mod transaction_limits {
    ...
    impl TransactionLimitTest {
        pub fn write_kv_stores(n: u32) -> Global<TransactionLimitTestComponent> {
        ...
        pub fn read_kv_stores(n: u32) -> Global<TransactionLimitTestComponent> {
            let kv_store = KeyValueStore::new();
            kv_store.insert(0, 0);
            for _i in 0..n {
                kv_store.get(&0);
...
mod transaction_limits_substate {
    ...
    impl TransactionLimitSubstateTest {
        pub fn write_large_value(size: u32) -> Global<TransactionLimitSubstateTestComponent> {
            let kv_store = KeyValueStore::new();

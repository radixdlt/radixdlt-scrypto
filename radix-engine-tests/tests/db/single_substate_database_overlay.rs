...
fn substates_written_to_root_database_can_be_read() {
    ...
    let db = DatabaseOverlay::new_unmergeable(&root);
...
fn substates_written_to_overlay_can_be_read_later() {
    ...
    let mut db = DatabaseOverlay::new_unmergeable(&root);
...
fn substates_written_on_a_staging_database_from_transactions_can_be_read_later() {
    ...
    let database = DatabaseOverlay::new_unmergeable(&root_database);
...
fn run_scenarios(
    check_callback: impl Fn(
        ...
        (
            &UnmergeableDatabaseOverlay<'_, InMemorySubstateDatabase>,
            &TransactionReceipt,
        ),
    ),
) {
    ...
    let db2 = DatabaseOverlay::new_unmergeable(&db2_root);

...
#[test]
fn account_try_deposit_batch_method_is_callable_with_out_owner_signature() {
    // Arrange
    for is_virtual in [true, false] {
        let mut test_runner = AccountDepositModesTestRunner::new(is_virtual);
        ...
        // Act
        let receipt =
            test_runner.free_tokens_from_faucet_to_account(DepositMethod::TryDepositBatch, false);
...
#[test]
fn account_try_deposit_unsafe_method_is_callable_with_out_owner_signature() {
    // Arrange
    for is_virtual in [true, false] {
        let mut test_runner = AccountDepositModesTestRunner::new(is_virtual);
        ...
        // Act
        let receipt =
            test_runner.free_tokens_from_faucet_to_account(DepositMethod::TryDepositUnsafe, false);
...
#[test]
fn account_try_deposit_batch_unsafe_method_is_callable_with_out_owner_signature() {
    // Arrange
    for is_virtual in [true, false] {
        let mut test_runner = AccountDepositModesTestRunner::new(is_virtual);
        ...
        let receipt = test_runner
            .free_tokens_from_faucet_to_account(DepositMethod::TryDepositBatchUnsafe, false);
...
fn changing_account_deposit_mode_is_callable_with_owner_signature() {
    ...
    for is_virtual in [true, false] {
        ...
        // Act
        let receipt =
            test_runner.transition_account_deposit_mode(AccountDepositsMode::AllowExisting, true);
...
fn changing_account_deposit_mode_is_not_callable_with_out_owner_signature() {
    ...
    for is_virtual in [true, false] {
        ...
        // Act
        let receipt =
            test_runner.transition_account_deposit_mode(AccountDepositsMode::AllowExisting, false);
...
fn allow_all_allows_for_all_resource_deposits() {
    ...
    for is_virtual in [true, false] {
        ...
        // Act
        let receipt =
            test_runner.mint_and_deposit(resource_address, DepositMethod::TryDepositUnsafe, false);
...
fn allow_existing_disallows_deposit_of_resources_on_deny_list() {
    ...
    for is_virtual in [true, false] {
        let mut test_runner = AccountDepositModesTestRunner::new(is_virtual);
        test_runner
            .transition_account_deposit_mode(AccountDepositsMode::AllowExisting, true)
            .expect_commit_success();
        test_runner
...
fn disallow_all_does_not_permit_deposit_of_any_resource() {
    ...
    for is_virtual in [true, false] {
        let mut test_runner = AccountDepositModesTestRunner::new(is_virtual);
        test_runner
            .transition_account_deposit_mode(AccountDepositsMode::DenyAll, true)
            .expect_commit_success();
...
impl AccountDepositModesTestRunner {
    ...
    pub fn mint_and_deposit(
        ...
        let (method, is_vec) = match deposit_method {
            ...
            DepositMethod::TryDeposit => (ACCOUNT_TRY_DEPOSIT_IDENT, false),
            DepositMethod::TryDepositUnsafe => (ACCOUNT_TRY_DEPOSIT_UNSAFE_IDENT, false),
            DepositMethod::DepositBatch => (ACCOUNT_DEPOSIT_BATCH_IDENT, true),
            DepositMethod::TryDepositBatch => (ACCOUNT_TRY_DEPOSIT_BATCH_IDENT, true),
            DepositMethod::TryDepositBatchUnsafe => (ACCOUNT_TRY_DEPOSIT_BATCH_UNSAFE_IDENT, true),
        };
    ...
    pub fn transition_account_deposit_mode(
        &mut self,
        deposit_mode: AccountDepositsMode,
        sign: bool,
    ) -> TransactionReceipt {
        let manifest = ManifestBuilder::new()
            .call_method(
    ...
    pub fn burn(&mut self, resource_address: ResourceAddress) {
        ...
        let manifest = ManifestBuilder::new()
            ...
            .try_deposit_batch(virtual_account)
            .build();

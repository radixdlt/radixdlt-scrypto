...
fn significant_protocol_updates_are_emitted_in_epoch_change_event() {
    ...
    let manifest = ManifestBuilder::new()
        ...
        .create_proof_from_account_of_non_fungibles(
            ...
            VALIDATOR_OWNER_BADGE,
            &indexset!(NonFungibleLocalId::bytes(validators_addresses[0].as_node_id().0).unwrap()),
        )
        .signal_protocol_update_readiness(validators_addresses[0], "a".repeat(32).as_str())
        .create_proof_from_account_of_non_fungibles(
            validators_owner_badge_holders[1],
            VALIDATOR_OWNER_BADGE,
            &indexset!(NonFungibleLocalId::bytes(validators_addresses[1].as_node_id().0).unwrap()),
        )
        ...
        .create_proof_from_account_of_non_fungibles(
            ...
            VALIDATOR_OWNER_BADGE,
            &indexset!(NonFungibleLocalId::bytes(validators_addresses[2].as_node_id().0).unwrap()),
        )
        ...
        .create_proof_from_account_of_non_fungibles(
            ...
            VALIDATOR_OWNER_BADGE,
            &indexset!(NonFungibleLocalId::bytes(validators_addresses[3].as_node_id().0).unwrap()),
        )

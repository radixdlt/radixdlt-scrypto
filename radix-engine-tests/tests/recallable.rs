...
fn test_recall_on_internal_vault() {
    ...
    let (component_address, _): (ComponentAddress, ResourceAddress) = receipt.expect_commit(true).output(1);
...
fn test_recall_on_received_direct_access_reference_which_is_same_as_self() {
    ...
    let (component_address, recallable): (ComponentAddress, ResourceAddress) = receipt.expect_commit(true).output(1);

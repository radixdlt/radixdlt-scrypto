...
//! transactions exists in this model. Instead, there exists a
//! [`TestRuntime`](crate::prelude::TestRuntime) struct that each test can instantiate instances of.
//! Each [`TestRuntime`](crate::prelude::TestRuntime) instance has a substate store, track, and
//! kernel. On top of that, [`TestRuntime`](crate::prelude::TestRuntime) implements the
//! [`ClientApi`](crate::prelude::ClientApi) trait. The [`TestRuntime`](crate::prelude::TestRuntime)
//! can be looked at as self-contained Radix Engine that’s exposed through the
//! [`ClientApi`](crate::prelude::ClientApi) and that has some other high-level helper methods as it
//! contains all of the layers of the engine. This means that:
//!
//! * A [`TestRuntime`](crate::prelude::TestRuntime) instance is a self-contained instance of the
//! Radix Engine and Kernel which are exposed through the [`ClientApi`](crate::prelude::ClientApi).
//! * Since [`TestRuntime`](crate::prelude::TestRuntime) implements the
//! [`ClientApi`](crate::prelude::ClientApi) it can be used as a substitute to ScryptoEnv from
//! Scrypto and the SystemService from native. This means that the simple interface seen in the
//! native_sdk crate can be used within tests.
//! * If each test has it’s own [`TestRuntime`](crate::prelude::TestRuntime) instance (they
//! instantiate that themselves if they need it), then tests have no shared dependencies and are
//! isolated.
...
//!
//! Once the [`TestRuntime`](crate::prelude::TestRuntime) has been instantiated we would get a
//! Kernel with two Call Frames:
...
//! will be made throughout the test. This Call Frame functions exactly like any other Call Frame,
//! it can own nodes, get messages from other Call Frames, and so on. As an example, say we invoke a
//! method on some node that returns a Bucket, this Bucket is now owned and visible to this Call
//! Frame. We are now able to call methods such as resource_address() and amount() on this Bucket
//! since it’s a node we own and the [`TestRuntime`](crate::prelude::TestRuntime) has a Heap.

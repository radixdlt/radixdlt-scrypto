...
impl TestRuntime {
    ...
    /// # Note
    ///
    /// * This method doesn't do direct-access calls and non-main module calls to provide a slightly
    /// simpler API.
    ...
    /// Invokes a method on a node with the provided typed arguments.
    ///
    /// This method is a typed version of the [`ClientObjectApi::call_method_advanced`] which
    /// Scrypto encodes the arguments and Scrypto decodes the returns on behalf of the caller. This
    /// method assumes that the caller is correct about the argument and return types and panics if
    /// the encoding or decoding fails.
    ///
    /// # Arguments
    ///
    /// * `node_id`: `T` - The node to invoke the method on. This is a generic argument that's
    /// fulfilled by any type that implements [`Into<NodeId>`], thus, any address type can be used.
    /// * `module_id`: [`ObjectModuleId`] - The module id of the node to invoke the method on.
    /// * `method_name`: [`&str`] - The name of the method to invoke.
    /// * `args`: `&I` - The arguments to invoke the method with. This is a generic arguments that
    /// is fulfilled by any type that implements [`ScryptoEncode`].
    /// * `direct_access`: [`bool`] - A boolean that determines whether the call is a direct access
    /// call or not.
    ///
    /// # Returns
    ///
    ...
    /// * Through an unwrap when calling [`scrypto_decode`] on the returns. This panics if the type
    /// could be decoded as the desired output type.
    pub fn call_method_advanced_typed<N, I, O>(
        &mut self,
        node_id: N,
    ...
    {
        let args = scrypto_encode(args).expect("Scrypto encoding of args failed");
        self.call_method_advanced(&node_id.into(), module_id, direct_access, method_name, args)
            .map(|rtn| scrypto_decode(&rtn).expect("Scrypto decoding of returns failed"))
    }

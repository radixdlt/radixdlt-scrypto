...
fn test_custom_type_values_are_valid() {
    ...
    // In particular, we only test types here which are only actually fully defined in `radix-engine-system-interface`.
    ...
    test_equivalence(
        ROYALTY_AMOUNT_TYPE,
        RoyaltyAmount::Usd(Decimal::from_str("1.6").unwrap()),
    );
    test_equivalence(
        ROYALTY_AMOUNT_TYPE,
        RoyaltyAmount::Xrd(Decimal::from_str("1.6").unwrap()),
    );
}
...
fn test_type_data_equivalent<T: ScryptoDescribe>(id: WellKnownTypeId) {
    ...
    let localized_type_data = localize_well_known_type_data::<ScryptoCustomSchema>(T::type_data());

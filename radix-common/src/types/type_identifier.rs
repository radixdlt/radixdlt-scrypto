...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Sbor)]
pub struct FullyScopedTypeId<T: Into<NodeId> + AsRef<NodeId>>(
    pub T,
    pub SchemaHash,
    pub LocalTypeId,
);

impl<T: Into<NodeId> + AsRef<NodeId>> FullyScopedTypeId<T> {
    pub fn into_general(self) -> FullyScopedTypeId<NodeId> {
        FullyScopedTypeId(self.0.into(), self.1, self.2)
    }
}
...
impl ScopedTypeId {
    pub fn under_node<T: Into<NodeId> + AsRef<NodeId>>(self, node: T) -> FullyScopedTypeId<T> {

    ...
    pub fn owned_node_ids(&self) -> Vec<RENodeId> {
        match self {
            KeyValueStoreEntrySubstate::Some(k, v) => {
                let (_, mut owns1, _) = IndexedScryptoValue::from_scrypto_value(k.clone()).unpack();
                let (_, owns2, _) = IndexedScryptoValue::from_scrypto_value(v.clone()).unpack();
                owns1.extend(owns2);
                owns1
            }
    ...
    pub fn references(&self) -> HashSet<RENodeId> {
        match self {
            KeyValueStoreEntrySubstate::Some(k, v) => {
                ...
                let (_, _, refs2) = IndexedScryptoValue::from_scrypto_value(v.clone()).unpack();
                refs1.extend(refs2);
                refs1
            }

...
macro_rules! internal_roles {
    ($role_struct:ident, $($role:ident => $rule:expr, $locked:ident;)* ) => ({
        let method_roles = $crate::internal_roles_struct!($role_struct, $($role => $rule, $locked;)*);
...
macro_rules! internal_roles_struct {
    ($role_struct:ident, $($role:ident => $rule:expr, $locked:ident;)* ) => ({
        $role_struct::<RoleDefinition> {
            $(
                $role: {
                    $crate::role_definition_entry!($rule, $locked)
                }
            ),*
        }
    });
...
macro_rules! roles_init_set_entry {
    ($roles:expr, $key:expr, $value:expr, updatable) => {{
        $roles.define_mutable_role($key, $value);
    }};
    ($roles:expr, $key:expr, $value:expr, locked) => {{
        $roles.define_immutable_role($key, $value);
    }};
}
...
macro_rules! roles_init {
    ...
    ( $($key:expr => $value:expr, $lock:ident;)* ) => ({
        ...
        $(
            $crate::roles_init_set_entry!(roles_init, $key, $value, $lock);
        )*
        roles_init
    });
}

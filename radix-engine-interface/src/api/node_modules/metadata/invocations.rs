...
pub trait MetadataVal: ScryptoEncode + ScryptoDecode {
...
impl MetadataInit {
    pub fn set_metadata<S: ToString, V: MetadataVal>(&mut self, key: S, value: V) {
        self.set(key.to_string(), value.to_metadata_value());
    }

    pub fn set_and_lock_metadata<S: ToString, V: MetadataVal>(&mut self, key: S, value: V) {
        self.set_and_lock(key.to_string(), value.to_metadata_value());
    }
}
...
macro_rules! metadata_init {
    () => ({
        radix_engine_interface::api::node_modules::metadata::MetadataInit::new()
    });
    ( $($key:expr => $value:expr, $lock:ident;)* ) => ({
        let mut metadata_init = radix_engine_interface::api::node_modules::metadata::MetadataInit::new();
        $(
            radix_engine_interface::metadata_init_set_entry!(metadata_init, $key, $value, $lock);
        )*
        metadata_init
    });
}

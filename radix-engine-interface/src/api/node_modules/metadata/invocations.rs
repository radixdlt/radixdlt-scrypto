...
impl ToMetadataEntry for EmptyMetadata {
    fn to_metadata_entry(self) -> Option<MetadataValue> {
        None
    }
}
...
macro_rules! metadata {
    ...
    });
    {
        ...
        let metadata_roles = metadata_roles!($($role => $rule $(, $updatable)? ;)*);
...

    {
        ...
        let roles = metadata_roles!($($role => $rule $(, $updatable:ident)? ;)*);
        ModuleConfig {
            init: metadata_init!(),
            roles,
        }
...
macro_rules! metadata_roles {
    {
        ...
        internal_roles!(MetadataRoles, $($role => $rule $(, $updatable)? ;)*)
    });
}

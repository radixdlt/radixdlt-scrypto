...
/// Marks the rust type that represents a non-fungible id, of any kind (i.e. Number, String and Bytes).
///
/// For now, it's `u32 -> Number`, `String -> String`, `Vec<u8> -> Bytes`, `u128 -> UUID`.
///
/// In the future, we will use dedicated types to represent each variant.
pub trait IsNonFungibleLocalId: Into<NonFungibleLocalId> {
    fn id_type() -> NonFungibleIdType;
}

impl IsNonFungibleLocalId for String {
    fn id_type() -> NonFungibleIdType {
        NonFungibleIdType::String
    }
}
impl IsNonFungibleLocalId for u64 {
    fn id_type() -> NonFungibleIdType {
        NonFungibleIdType::Number
    }
}
impl IsNonFungibleLocalId for Vec<u8> {
    fn id_type() -> NonFungibleIdType {
        NonFungibleIdType::Bytes
    }
}
impl IsNonFungibleLocalId for u128 {
    fn id_type() -> NonFungibleIdType {
        NonFungibleIdType::UUID
    }
}

/// Marks the rust type that represents a non-fungible id, of non-auto-generated kind (i.e. Number, String and Bytes).
///
/// For now, it's `u32 -> Number`, `String -> String`, `Vec<u8> -> Bytes`.
///
/// In the future, we will use dedicated types to represent each variant.
pub trait IsNonAutoGeneratedNonFungibleLocalId: IsNonFungibleLocalId {}

impl IsNonAutoGeneratedNonFungibleLocalId for String {}
impl IsNonAutoGeneratedNonFungibleLocalId for u64 {}
impl IsNonAutoGeneratedNonFungibleLocalId for Vec<u8> {}

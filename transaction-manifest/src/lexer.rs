...
impl Lexer {
    ...
    fn tokenize_identifier(&mut self) -> Result<Token, LexerError> {
        ...
        match id.as_str() {
            ...
            "ADD_TO_WORKTOP" => Ok(TokenKind::AddToWorktop),
            "ASSERT_WORKTOP" => Ok(TokenKind::AssertWorktop),
            "ASSERT_WORKTOP_BY_AMOUNT" => Ok(TokenKind::AssertWorktopByAmount),
            "ASSERT_WORKTOP_BY_IDS" => Ok(TokenKind::AssertWorktopByIds),
            "TAKE_FROM_AUTH_ZONE" => Ok(TokenKind::TakeFromAuthZone),
            "ADD_TO_AUTH_ZONE" => Ok(TokenKind::AddToAuthZone),

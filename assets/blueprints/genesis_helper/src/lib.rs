...
mod genesis_helper {
    ...
    impl GenesisHelper {
        ...
        fn create_validator(&mut self, validator: GenesisValidator) {
            let xrd_payment = ResourceManager(XRD).new_empty_bucket(&mut ScryptoEnv).unwrap();
            let (validator_address, owner_token_bucket) = ConsensusManager(self.consensus_manager)
                .create_validator(validator.key, validator.fee_factor, xrd_payment, &mut ScryptoEnv)
                .unwrap();

...
mint_burn_auth=`$resim new-resource-fixed 1 | tee /dev/tty | awk '/ResourceDef:/ {print $NF}'`
resource_def=`$resim new-resource-mutable $mint_burn_auth | tee /dev/tty | awk '/ResourceDef:/ {print $NF}'`
$resim mint 777 $resource_def $mint_burn_auth --signers $account_key

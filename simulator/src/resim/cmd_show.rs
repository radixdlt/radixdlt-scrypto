...
impl Show {
    pub fn run(&self) -> Result<(), Error> {
        ...
        if let Ok(package_address) = PackageAddress::from_str(&self.id) {
            dump_package(package_address, &ledger).map_err(Error::LedgerDumpError)
        } else if let Ok(component_address) = ComponentAddress::from_str(&self.id) {
            dump_component(component_address, &ledger).map_err(Error::LedgerDumpError)
        } else if let Ok(resource_address) = ResourceAddress::from_str(&self.id) {
            dump_resource_def(resource_address, &ledger).map_err(Error::LedgerDumpError)
        } else {

use crate::{internal_prelude::*, CustomExtension, CustomSchema, Decoder as _, Encoder as _, Describe, VecDecoder, VecEncoder};
...
pub fn vec_encode<E: CustomExtension, T: VecEncode<E::CustomValueKind> + ?Sized>(value: &T, max_depth: usize) -> Result<Vec<u8>, EncodeError> {
...
pub fn vec_decode<E: CustomExtension, T: VecDecode<E::CustomValueKind>>(buf: &[u8], max_depth: usize) -> Result<T, DecodeError> {

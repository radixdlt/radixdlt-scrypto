...
mod tests {
    ...
    fn encode_decode_size(size: usize) -> Result<(), DecodeError> {
        ...
        let mut enc = BasicEncoder::new(&mut bytes, 100);
        enc.write_size(size).unwrap();

        let mut dec = BasicDecoder::new(&bytes, 100);
    ...
    pub fn test_vlq_too_large() {
        let mut dec = BasicDecoder::new(&[0xff, 0xff, 0xff, 0xff, 0x00], 100);
    ...
    pub fn test_generic_array() {
        ...
        // Encode
        let mut bytes = Vec::with_capacity(512);
        let mut encoder = BasicEncoder::new(&mut bytes, 100);
        encoder.encode(&value1).unwrap();

        let mut decoder = BasicDecoder::new(&bytes, 100);

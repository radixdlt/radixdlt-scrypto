...
pub enum TypedTraversalEvent<'de, C: CustomTraversal> {
    ...
    TerminalValue(TypedLocatedDecoding<TerminalValueRef<'de, C::CustomTerminalValueRef>>),
    TerminalValueBatch(
        TypedLocatedDecoding<TerminalValueBatchRef<'de, C::CustomTerminalValueBatchRef>>,
    ),
    PayloadEnd(Location),
...
pub enum TypeMismatchError<X: CustomValueKind> {
    MismatchingType { expected: LocalTypeIndex, actual: ValueKind<X> },
    MismatchingChildElementType { expected: LocalTypeIndex, actual: ValueKind<X> },
    MismatchingChildKeyType { expected: LocalTypeIndex, actual: ValueKind<X> },
    MismatchingChildValueType { expected: LocalTypeIndex, actual: ValueKind<X> },
    MismatchingTupleLength { expected: usize, actual: usize, type_index: LocalTypeIndex },
    MismatchingEnumVariantLength { expected: usize, actual: usize, variant: u8, type_index: LocalTypeIndex },
    UnknownEnumVariant { type_index: LocalTypeIndex, variant: u8 },
}

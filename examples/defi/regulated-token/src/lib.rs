    ...
    impl RegulatedToken {
        ...
        pub fn advance_stage(&mut self) {
            ...
            if self.current_stage == 1 {
                ...
                token_def.disable_flags(
                    MINTABLE | RESTRICTED_TRANSFER | SHARED_METADATA_MUTABLE,
                    auth.clone(),
                );
        ...
        pub fn buy_token(&mut self, quantity: Decimal, mut payment: Bucket) -> (Bucket, Bucket) {
            ...
            if extra_demand <= 0.into() {
                ...
                let tokens = self
                    .internal_authority
                    .authorize(|auth| self.token_supply.take_with_auth(quantity, auth));
                ...
                let existing_tokens = self
                    .internal_authority
                    .authorize(|auth| self.token_supply.take_all_with_auth(auth));

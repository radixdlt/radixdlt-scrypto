...
blueprint! {
    ...
    impl PresentList {
        ...
        pub fn add(&mut self, present_name: String, list_badge: BucketRef) {
            ...
            let list = self.lists.get_mut(&list_address).unwrap();
            ...
            // Append the present to the list
            list.push(present_name);
        ...
        pub fn remove(&mut self, present_name: String, list_badge: BucketRef) {
            ...
            let index = list.iter().position(|x| *x == present_name).unwrap();
            list.remove(index);
        }

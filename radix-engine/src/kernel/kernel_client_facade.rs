    ...
    M: BaseModule<R>,
{
    fn sys_drop_node(&mut self, node_id: RENodeId) -> Result<(), RuntimeError> {
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
    ...
    M: BaseModule<R>,
{
    fn sys_lock_substate(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            ...
            api.lock_substate(node_id, NodeModuleId::SELF, offset, flags)
        })
    }

    fn sys_read_substate(&mut self, lock_handle: LockHandle) -> Result<Vec<u8>, RuntimeError> {
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            api.get_ref(lock_handle)
    ...
    fn sys_write_substate(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            let offset = api.get_lock_info(lock_handle)?.offset;
            let substate = RuntimeSubstate::decode_from_buffer(&offset, &buffer)?;
            let mut substate_mut = api.get_ref_mut(lock_handle)?;
    ...
    M: BaseModule<R>,
{
    fn fn_identifier(&mut self) -> Result<FnIdentifier, RuntimeError> {
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            Ok(api.current_frame.actor.identifier.clone())
        })
    }
}
    ...
    M: BaseModule<R>,
{
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            ...
            match call_table_invocation {
                CallTableInvocation::Native(native_invocation) => {
                    invoke_native_fn(native_invocation, api)
    ...
    fn call_native<N: SerializableInvocation>(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            ...
            let invocation =
                scrypto_encode(&invocation).expect("Failed to encode native invocation");
            let return_data = api.call_native_raw(native_fn, invocation)?;
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            ...
            match invocation {
                CallTableInvocation::Native(native_invocation) => Ok(scrypto_encode(
                    invoke_native_fn(native_invocation, api)?.as_ref(),
                )
                .expect("Failed to encode native fn return")),
                CallTableInvocation::Scrypto(scrypto_invocation) => api
    ...
    M: BaseModule<R>,
{
    ...
    fn new_component(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            // Allocate node id
            let node_id = api.allocate_node_id(RENodeType::Component)?;
            ...
            let package_address = match api.current_frame.actor.identifier.clone() {
    ...
    fn globalize_component(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            let node_id = api.allocate_node_id(RENodeType::GlobalComponent)?;
    ...
    fn call_method(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            // TODO: Use execution mode?
            let invocation = resolve_method(receiver, method_name, &args, api)?;
            match invocation {
                CallTableInvocation::Native(native_invocation) => Ok(scrypto_encode(
                    invoke_native_fn(native_invocation, api)?.as_ref(),
                )
    ...
    fn new_key_value_store(&mut self) -> Result<KeyValueStoreId, RuntimeError> {
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            ...
            api.create_node(node_id, RENodeInit::KeyValueStore, btreemap!())?;
    ...
    M: BaseModule<R>,
{
    fn consume_cost_units(&mut self, units: u32) -> Result<(), RuntimeError> {
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            api.module
                .on_wasm_costing(&api.current_frame, &mut api.heap, api.track, units)
    ...
    fn lock_fee(
        ...
        self.execute_in_mode::<_, _, RuntimeError>(ExecutionMode::System, |api| {
            let rtn = api.module.on_lock_fee(
                &api.current_frame,
                &mut api.heap,
                api.track,
                vault_id,

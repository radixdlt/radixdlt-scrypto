...
impl KernelModule for KernelModuleMixer {
    ...
    fn on_read_bucket<Y: KernelModuleApi<RuntimeError>>(api: &mut Y) {
        ...
        if modules.contains(EnabledModules::COSTING) {
            CostingModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::NODE_MOVE) {
            NodeMoveModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::AUTH) {
            AuthModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::LOGGER) {
            LoggerModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::TRANSACTION_RUNTIME) {
            TransactionRuntimeModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::EXECUTION_TRACE) {
            ExecutionTraceModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::TRANSACTION_LIMITS) {
            TransactionLimitsModule::on_read_bucket(api);
        }
        if modules.contains(EnabledModules::EVENTS) {
            EventsModule::on_read_bucket(api);
        }
    }
    ...
    fn on_read_proof<Y: KernelModuleApi<RuntimeError>>(api: &mut Y) {
        ...
        if modules.contains(EnabledModules::COSTING) {
            CostingModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::NODE_MOVE) {
            NodeMoveModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::AUTH) {
            AuthModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::LOGGER) {
            LoggerModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::TRANSACTION_RUNTIME) {
            TransactionRuntimeModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::EXECUTION_TRACE) {
            ExecutionTraceModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::TRANSACTION_LIMITS) {
            TransactionLimitsModule::on_read_proof(api);
        }
        if modules.contains(EnabledModules::EVENTS) {
            EventsModule::on_read_proof(api);
        }
    }
}

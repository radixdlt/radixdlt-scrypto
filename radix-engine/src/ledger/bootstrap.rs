...
fn create_genesis<S: ReadableSubstateStore>(mut track: Track<S>) -> TrackReceipt {
    let system_package = new_extracted_package(include_bytes!("../../../assets/system.wasm").to_vec()).unwrap();
    ...
    let account_package = new_extracted_package(include_bytes!("../../../assets/account.wasm").to_vec()).unwrap();

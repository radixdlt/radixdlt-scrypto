...
fn create_genesis<S: ReadableSubstateStore>(mut track: Track<S>) -> TrackReceipt {
    let system_package = extract_package(include_bytes!("../../../assets/system.wasm").to_vec()).unwrap();
    ...
    let account_package = extract_package(include_bytes!("../../../assets/account.wasm").to_vec()).unwrap();

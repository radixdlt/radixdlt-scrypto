...
impl RENode {
    ...
    pub fn prepare_move_downstream(
        ...
        match self {
            ...
            | RENode::Worktop(..)
            | RENode::System(..)
            | RENode::Global(..) => Err(RuntimeError::KernelError(
    ...
    pub fn prepare_move_upstream(&mut self, self_node_id: RENodeId) -> Result<(), RuntimeError> {
        match self {
            ...
            RENode::Component(..) => Ok(()),
            RENode::Vault(..) => Ok(()),
            RENode::AuthZone(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::ResourceManager(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::KeyValueStore(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::NonFungibleStore(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::Package(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::Worktop(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::System(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),
            )),
            RENode::Global(..) => Err(RuntimeError::KernelError(
                KernelError::CantMoveUpstream(self_node_id),

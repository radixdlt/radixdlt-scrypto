...
pub struct TrackNodeDag<'s, S: ReadableSubstateStore + WriteableSubstateStore> {
    parent: &'s mut S,
    nodes: HashMap<u64, TrackNode>,
    cur_id: u64,
}

impl<'s, S: ReadableSubstateStore + WriteableSubstateStore> TrackNodeDag<'s, S> {
    ...
    pub fn new_branch(&mut self, parent_id: u64) -> u64 {
        ...
        self.nodes.insert(self.cur_id, TrackNode::new(parent_id));
        self.cur_id
    }

    pub fn get_output_store<'t>(&'t mut self, id: u64) -> TrackedStore<'t, 's, S> {
        if self.nodes.get(&id).unwrap().locked {
            panic!("Should not write to locked node");
        }

        TrackedStore { dag: self, id }
    }

    fn remove_node(&mut self, id: u64, remove_children: bool) -> TrackNode {
...
pub struct TrackedStore<'t, 's, S: ReadableSubstateStore + WriteableSubstateStore> {
    dag: &'t mut TrackNodeDag<'s, S>,
    id: u64,
}

impl<'t, 's, S: ReadableSubstateStore + WriteableSubstateStore> TrackedStore<'t, 's, S> {

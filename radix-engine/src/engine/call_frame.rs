    ...
    fn to_snode_state(&mut self) -> SNodeState {
        match self {
            ...
            Borrowed(ref mut borrowed) => match borrowed {
                ...
                BorrowedSNodeState::CallerAuthZone(_) => todo!(),
                BorrowedSNodeState::AuthZoneManager(_) => todo!(),
            },
        }
    }
}
...
impl BorrowedSNodeState {
        ...
        I: WasmInstance,
    {
        match self {
            ...
            BorrowedSNodeState::CallerAuthZone(auth_zone) => {
                frame.shadow_auth_zone = Some(auth_zone);
            }
            BorrowedSNodeState::AuthZoneManager(_) => todo!(),
        ...
        let output = match snode {
            ...
            SNodeState::AuthZoneManager(current_auth_zone) => {
                match method_name {
                    ...
                    _ => Err(RuntimeError::UnknownMethod(method_name.to_string())),
                }
            }
            SNodeState::CallerAuthZone(_) => todo!(),
        }?;
    ...
    I: WasmInstance,
{
    ...
    fn invoke_snode2(
        ...
        let (mut loaded_snode, method_auths) = match &snode_ref {
            ...
            SNodeRef::CallerAuthZoneRef => {
                ...
                if let Some(auth_zone) = self.shadow_auth_zone.take() {
                    Ok((
                        Borrowed(BorrowedSNodeState::CallerAuthZone(auth_zone)),
                        vec![],
                    ))
                } else {
                    Err(RuntimeError::AuthZoneDoesNotExist)
            ...
            SNodeRef::AuthZoneManager => {
                if let Some(auth_zone) = self.auth_zone.take() {
                    Ok((
                        Borrowed(BorrowedSNodeState::AuthZoneManager(auth_zone)),
                        vec![],
                    ))
                } else {
                    Err(RuntimeError::AuthZoneDoesNotExist)
        ...
        if !method_auths.is_empty() {
            ...
            process_shadow_auth_zone = match self.auth_zone_stack.len() {
                0 => None,
                _ => Some(AuthZone::new()),
            };
        ...
        if let Some(auth_zone) = process_shadow_auth_zone {
            frame.shadow_auth_zone = Option::Some(auth_zone);
        }
        ...
        if let Some(mut auth_zone) = frame.shadow_auth_zone.take() {
            auth_zone.clear();
        }
    ...
    fn check_access_rule(
        ...
        let is_authorized = method_authorization
            .check(&[&simulated_auth_zone], &mut None)
            .is_ok();
        simulated_auth_zone
            .main(

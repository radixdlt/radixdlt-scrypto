...
use crate::system::system_modules::execution_trace::{BucketSnapshot, ExecutionTraceModule, ProofSnapshot};
...
use crate::track::{NodeSubstates, to_state_updates, Track};
...
use crate::kernel::call_frame::{CallFrameMessage, NodeVisibility};
...
pub fn execute_transaction<S: SubstateDatabase, V: SystemCallbackObject + Clone>(
    ...
    execute_transaction_with_wrapper::<S, V, SystemConfig<V>>(substate_db, vm, costing_parameters, execution_config, transaction)
}

pub fn execute_transaction_with_wrapper<S: SubstateDatabase, V: SystemCallbackObject + Clone, T: WrappedSystem<V>>(
    substate_db: &S,
    vm: V,
    costing_parameters: &CostingParameters,
    execution_config: &ExecutionConfig,
...
impl<'a, M: KernelCallbackObject, K: KernelApi<RandomCallbackError<M>>> KernelNodeApi for WrappedKernelApi<'a, M, K> {
...
impl<'a, M: KernelCallbackObject, Y: KernelApi<RandomCallbackError<M>>> KernelSubstateApi<M::LockData> for WrappedKernelApi<'a, M, Y> {
    fn kernel_mark_substate_as_transient(&mut self, node_id: NodeId, partition_num: PartitionNumber, key: SubstateKey) -> Result<(), RuntimeError> {
        self.api.kernel_mark_substate_as_transient(node_id, partition_num, key)
    }

    fn kernel_open_substate_with_default<F: FnOnce() -> IndexedScryptoValue>(&mut self, node_id: &NodeId, partition_num: PartitionNumber, substate_key: &SubstateKey, flags: LockFlags, default: Option<F>, lock_data: M::LockData) -> Result<SubstateHandle, RuntimeError> {
        self.api.kernel_open_substate_with_default(node_id, partition_num, substate_key, flags, default, lock_data)
    }

    fn kernel_get_lock_data(&mut self, lock_handle: SubstateHandle) -> Result<M::LockData, RuntimeError> {
    ...
    fn kernel_set_substate(&mut self, node_id: &NodeId, partition_num: PartitionNumber, substate_key: SubstateKey, value: IndexedScryptoValue) -> Result<(), RuntimeError> {
        self.api.kernel_set_substate(node_id, partition_num, substate_key, value)
    }
    ...
    fn kernel_remove_substate(&mut self, node_id: &NodeId, partition_num: PartitionNumber, substate_key: &SubstateKey) -> Result<Option<IndexedScryptoValue>, RuntimeError> {
        self.api.kernel_remove_substate(node_id, partition_num, substate_key)
    }
    ...
    fn kernel_scan_sorted_substates(&mut self, node_id: &NodeId, partition_num: PartitionNumber, count: u32) -> Result<Vec<(SortedKey, IndexedScryptoValue)>, RuntimeError> {
        self.api.kernel_scan_sorted_substates(node_id, partition_num, count)
    }
    ...
    fn kernel_scan_keys<K: SubstateKeyContent + 'static>(&mut self, node_id: &NodeId, partition_num: PartitionNumber, count: u32) -> Result<Vec<SubstateKey>, RuntimeError> {
        self.api.kernel_scan_keys::<K>(node_id, partition_num, count)
    }
    ...
    fn kernel_drain_substates<K: SubstateKeyContent + 'static>(&mut self, node_id: &NodeId, partition_num: PartitionNumber, count: u32) -> Result<Vec<(SubstateKey, IndexedScryptoValue)>, RuntimeError> {
        self.api.kernel_drain_substates::<K>(node_id, partition_num, count)
    }
}

impl<'a, M: KernelCallbackObject + 'a, K: KernelApi<RandomCallbackError<M>>> KernelInvokeApi<M::CallFrameData> for WrappedKernelApi<'a, M, K> {
    fn kernel_invoke(&mut self, invocation: Box<KernelInvocation<M::CallFrameData>>) -> Result<IndexedScryptoValue, RuntimeError> {
        self.api.kernel_invoke(invocation)
    }
}

impl<'a, M: KernelCallbackObject, K: KernelApi<RandomCallbackError<M>>> KernelInternalApi<M> for WrappedKernelApi<'a, M, K> {
...
impl<'a, M: KernelCallbackObject, K: KernelApi<RandomCallbackError<M>>> KernelApi<M> for WrappedKernelApi<'a, M, K> {

}

pub struct WrappedKernelInternalApi<'a, M: KernelCallbackObject + 'a, K: KernelInternalApi<RandomCallbackError<M>>> {
    api: &'a mut K,
    phantom: PhantomData<M>,
}

impl<'a, M: KernelCallbackObject, K: KernelInternalApi<RandomCallbackError<M>>> KernelInternalApi<M> for WrappedKernelInternalApi<'a, M, K> {
    fn kernel_get_system_state(&mut self) -> SystemState<'_, M> {
        let state = self.api.kernel_get_system_state();
        SystemState {
            system: &mut state.system.callback_object,
...
impl<'a, K: KernelCallbackObject + 'a> KernelCallbackObject for RandomCallbackError<K> {
    type LockData = K::LockData;
    type CallFrameData = K::CallFrameData;

    fn on_init<Y>(api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
    ...
    fn start<Y>(api: &mut Y, manifest_encoded_instructions: &[u8], pre_allocated_addresses: &Vec<PreAllocatedAddress>, references: &IndexSet<Reference>, blobs: &IndexMap<Hash, Vec<u8>>) -> Result<Vec<u8>, RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::start(&mut api, manifest_encoded_instructions, pre_allocated_addresses, references, blobs)
    }

    fn on_teardown<Y>(api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
    ...
    fn on_create_node<Y>(api: &mut Y, event: CreateNodeEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_create_node(&mut api, event)
    }

    fn on_drop_node<Y>(api: &mut Y, event: DropNodeEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_drop_node(&mut api, event)
    }

    fn on_move_module<Y>(api: &mut Y, event: MoveModuleEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_move_module(&mut api, event)
    }

    fn on_open_substate<Y>(api: &mut Y, event: OpenSubstateEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_open_substate(&mut api, event)
    }

    fn on_close_substate<Y>(api: &mut Y, event: CloseSubstateEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_close_substate(&mut api, event)
    }

    fn on_read_substate<Y>(api: &mut Y, event: ReadSubstateEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
        K::on_read_substate(&mut api, event)
    }

    fn on_write_substate<Y>(api: &mut Y, event: WriteSubstateEvent) -> Result<(), RuntimeError> where Y: KernelInternalApi<Self> {
        let mut api = wrapped_internal_api!(api);
    ...
    fn before_invoke<Y>(invocation: &KernelInvocation<Self::CallFrameData>, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::before_invoke(invocation, &mut api)
    }

    fn after_invoke<Y>(output: &IndexedScryptoValue, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::after_invoke(output, &mut api)
    }

    fn on_execution_start<Y>(api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::on_execution_start(&mut api)
    }

    fn on_execution_finish<Y>(message: &CallFrameMessage, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::on_execution_finish(message, &mut api)
    }

    fn on_allocate_node_id<Y>(entity_type: EntityType, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::on_allocate_node_id(entity_type, &mut api)
    }

    fn invoke_upstream<Y>(args: &IndexedScryptoValue, api: &mut Y) -> Result<IndexedScryptoValue, RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::invoke_upstream(args, &mut api)
    }

    fn auto_drop<Y>(nodes: Vec<NodeId>, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
    ...
    fn on_mark_substate_as_transient(&mut self, node_id: &NodeId, partition_number: &PartitionNumber, substate_key: &SubstateKey) -> Result<(), RuntimeError> {
        self.callback_object.on_mark_substate_as_transient(node_id, partition_number, substate_key)
    }

    fn on_substate_lock_fault<Y>(node_id: NodeId, partition_num: PartitionNumber, offset: &SubstateKey, api: &mut Y) -> Result<bool, RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::on_substate_lock_fault(node_id, partition_num, offset, &mut api)
    }

    fn on_drop_node_mut<Y>(node_id: &NodeId, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = wrapped_api!(api);
        K::on_drop_node_mut(node_id, &mut api)
    }

    fn on_move_node<Y>(node_id: &NodeId, is_moving_down: bool, is_to_barrier: bool, destination_blueprint_id: Option<BlueprintId>, api: &mut Y) -> Result<(), RuntimeError> where Y: KernelApi<Self> {
        let mut api = WrappedKernelApi {
            api,
            phantom: PhantomData::default(),
        };
        K::on_move_node(node_id, is_moving_down, is_to_barrier, destination_blueprint_id, &mut api)
    }
}
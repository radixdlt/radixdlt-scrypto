...
impl ExecutableInvocation for AccountNewWithResourceInvocation {
    ...
    where
        Self: Sized,
    {
        let actor = ResolvedActor::function(NativeFn::Account(AccountFn::NewWithResource));
        ...
        call_frame_update.add_ref(RENodeId::Global(GlobalAddress::Resource(
            bucket_resource_address(deref, self.bucket)?,
        )));
...
impl Executor for AccountNewWithResourceInvocation {
        ...
        Y: SystemApi + EngineApi<RuntimeError> + InvokableModel<RuntimeError>,
    {
        ...
        // Creating a new vault and depositing the passed bucket into it
        let resource_address = bucket_resource_address(api, self.bucket)?;
        let vault = {
            let mut vault = Vault::sys_new(resource_address, api)?;
            vault.sys_put(Bucket(self.bucket), api)?;
            vault
        };

        // Adding the vault to the key-value store
        {
            let encoded_key = scrypto_encode(&resource_address).expect("Impossible Case!");
            let encoded_value = scrypto_encode(&Own::Vault(vault.0)).expect("Impossible Case!");

            let node_id = kv_store_id;

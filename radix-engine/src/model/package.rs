...
use crate::wasm::{
    NopScryptoRuntime, ScryptoModule, ScryptoWasmExecutor, ScryptoWasmValidator, WasmiEngine,
    WasmiScryptoModule,
};
use crate::wasm::{WasmValidationError, ENGINE_FUNCTION_INDEX};
use sbor::*;
...
impl Package {
    ...
    pub fn call_abi(&self, blueprint_name: &str) -> Result<ScryptoValue, RuntimeError> {
        ...
        let result = module
            .module_ref
            .invoke_export(&export_name, &[], &mut NopExternals);
        let rtn = result
            .map_err(|e| {

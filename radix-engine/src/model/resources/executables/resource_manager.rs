...
impl<W: WasmEngine> ExecutableInvocation<W> for ResourceManagerCreateWithOwnerInvocation {
    type Exec = ResourceManagerCreateInvocation;
    ...
    fn resolve<D: ResolverApi<W>>(
        ...
        let executor = ResourceManagerCreateInvocation {
...
impl Executor for ResourceManagerCreateVaultExecutable {
    ...
    fn execute<'a, Y>(self, api: &mut Y) -> Result<(Vault, CallFrameUpdate), RuntimeError>
    where
        Y: SystemApi,
    {
        let offset = SubstateOffset::ResourceManager(ResourceManagerOffset::ResourceManager);

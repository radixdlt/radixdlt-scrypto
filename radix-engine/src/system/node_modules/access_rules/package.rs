...
impl AccessRulesNativePackage {
    ...
    pub(crate) fn get_authorization_for_set_method_access_rule_and_mutability<Y>(
        ...
        where
            Y: KernelNodeApi + KernelSubstateApi,
    {
        let input: AccessRulesSetMethodAccessRuleAndMutabilityInput = input.as_typed().map_err(|e| {
            RuntimeError::InterpreterError(InterpreterError::ScryptoInputDecodeError(e))
        })?;
    ...
    pub(crate) fn get_authorization_for_set_group_access_rule_and_mutability<Y>(
        ...
        where
            Y: KernelNodeApi + KernelSubstateApi,
    {
        let input: AccessRulesSetGroupAccessRuleAndMutabilityInput = input.as_typed().map_err(|e| {
            RuntimeError::InterpreterError(InterpreterError::ScryptoInputDecodeError(e))
        })?;
            ...
            Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        let access_rules_substate: &mut MethodAccessRulesSubstate =
            api.kernel_get_substate_ref_mut(handle)?;
        let access_rules = &mut access_rules_substate.access_rules;
        access_rules.set_group_access_rule_and_mutability(input.name.clone(), input.rule.clone(), input.mutability.clone());

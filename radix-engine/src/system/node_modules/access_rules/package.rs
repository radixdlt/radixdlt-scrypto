...
impl CycleCheck<AuthorityKey> for NodeAuthorityRules {
    fn get_references(
        ...
        let mutability_references = if let Some(access_rule) = self.mutability.get(authority) {
            access_rule.get_referenced_authorities()
        } else {
            index_set_new()
        };
        ...
        match authority {
            AuthorityKey::Owner => {
                ...
                for mutability_ref in mutability_references {
                    match mutability_ref {
                        AuthorityRule::Owner => {}
                        AuthorityRule::Custom(..) => {
                            return Err(
                                CycleCheckError::OwnerAuthorityMutabilityHasInvalidReferences,
                            );
                        }
                    }
                }
...
impl CycleCheck<AuthorityKey> for AuthorityRules {
    fn get_references(
        ...
        let (references, mutability_references) =
            if let Some((access, mutability)) = self.rules.get(authority) {
                (
                    access.get_referenced_authorities(),
                    mutability.get_referenced_authorities(),
                )
            } else {
                return Err(CycleCheckError::MissingAuthority(authority.clone()));
            };
        ...
        if !inner_blueprint {
            ...
            // Metadata
            {
                authority_rules.set_rule(
                    AuthorityKey::metadata(METADATA_SET_IDENT),
                    rule!(require(METADATA_AUTHORITY)),
                    rule!(deny_all),
                );
            ...
            {
                authority_rules.set_rule(
                    AuthorityKey::royalty(COMPONENT_ROYALTY_SET_ROYALTY_CONFIG_IDENT),
                    rule!(require(ROYALTY_AUTHORITY)),
                    rule!(deny_all),
                );
                authority_rules.set_rule(
                    AuthorityKey::royalty(COMPONENT_ROYALTY_CLAIM_ROYALTY_IDENT),
                    rule!(require(ROYALTY_AUTHORITY)),
                    rule!(deny_all),
                );
...
impl AccessRulesNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ACCESS_RULES_CREATE_IDENT => {
                ...
                let rtn = Self::create(input.authority_rules, input.inner_blueprint_rules, api)?;
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        let access_rules = NodeAuthorityRules::create(false, authority_rules).map_err(|e| {
        ...
        for (blueprint, authority_rules) in child_blueprint_rules {
            let inner_access_rules =
                NodeAuthorityRules::create(true, authority_rules).map_err(|e| {
                    RuntimeError::ModuleError(ModuleError::AuthError(AuthError::CycleCheckError(e)))
                })?;

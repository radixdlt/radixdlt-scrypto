...
impl NodeAuthorizationRules {
    ...
    fn create(
        ...
        for (method, entry) in method_permissions_and_mutability {
            ...
            method_permission_mutability.insert(method, entry.mutable);
        }
...
impl AccessRulesNativePackage {
    ...
    pub fn authorization<Y: KernelSubstateApi<L>, L: Default>(
        ...
        let permission = match ident {
            ACCESS_RULES_UPDATE_METHOD_IDENT => {
                ...
                Self::update_method_authorization(
                    node_id,
                    input.object_key,
                    &input.method_key,
                    api,
            ...
            ACCESS_RULES_UPDATE_ROLE_IDENT => {
                ...
                Self::update_role_authorization(node_id, &input.role_key, api)?
            }
    ...
    fn update_method_authorization<Y, L: Default>(
        receiver: &NodeId,
        object_key: ObjectKey,
        method_key: &MethodKey,
        api: &mut Y,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (authority, entry) in role_definitions.rules {
            ...
            protected_roles.insert(authority, entry.mutable);
        }
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if !access_rules.method_permissions.contains_key(&method_key) {
            return Err(RuntimeError::ApplicationError(
                ApplicationError::AccessRulesError(AccessRulesError::MethodDoesNotExist(
                    object_key, method_key,
                )),
            ));
        }

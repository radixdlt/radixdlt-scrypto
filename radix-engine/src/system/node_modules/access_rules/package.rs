...
pub struct NodeAuthorityRules {
    ...
    pub protected_methods: BTreeMap<MethodKey, MethodPermission>, // Should this be optional?
...
impl NodeAuthorityRules {
    ...
    fn create(
        mut protected_methods: BTreeMap<MethodKey, MethodPermission>,
        ...
        if !inner_blueprint {
            ...
            {
                // Always public
                protected_methods.insert(MethodKey::new(
                    ObjectModuleId::Metadata,
                    METADATA_GET_IDENT,
                ), MethodPermission::Public);

                let metadata_set = MethodKey::new(ObjectModuleId::Metadata, METADATA_SET_IDENT);
                if !protected_methods.contains_key(&metadata_set) {
                    protected_methods.insert(metadata_set, MethodPermission::Protected(RoleList::none()));
                }
            }

            {
                ...
                if !protected_methods.contains_key(&royalty_set) {
                    let mut role_list = RoleList::none();
                    if authority_rules.rules.contains_key(&RoleKey::new("owner")) {
                        role_list.insert("owner");
                    }
                    protected_methods.insert(royalty_set, MethodPermission::Protected(role_list));
                }
                let royalty_claim = MethodKey::new(
                    ObjectModuleId::Royalty,
                    COMPONENT_ROYALTY_CLAIM_ROYALTY_IDENT,
                );
                if !protected_methods.contains_key(&royalty_claim) {
                    let mut role_list = RoleList::none();
                    if authority_rules.rules.contains_key(&RoleKey::new("owner")) {
                        role_list.insert("owner");
                    }
                    protected_methods.insert(royalty_claim, MethodPermission::Protected(role_list));
                }
            }
        }
    ...
    pub fn get_authority_mutability(&self, key: &RoleKey) -> RoleList {
...
impl AccessRulesNativePackage {
    ...
    pub(crate) fn create<Y>(
        protected_module_methods: BTreeMap<MethodKey, MethodPermission>,
    ...
    {
        let access_rules =
            NodeAuthorityRules::create(protected_module_methods, false, role_definitions).map_err(
                |e| {

...
impl CycleCheck<AuthorityKey> for NodeAuthorityRules {
    fn get_references(
        ...
        let references = references.into_iter()
            .map(|s| AuthorityKey::module(s.as_str()))
            .collect();
...
impl NodeAuthorityRules {
    ...
    fn create(
        ...
        if !inner_blueprint {
            ...
            let owner_authority = AuthorityKey::module(OWNER_AUTHORITY);
            if !authority_rules.rules.contains_key(&owner_authority) {
                authority_rules.set_main_rule(OWNER_AUTHORITY, AccessRule::DenyAll, AccessRule::DenyAll)
            }

            let metadata_authority = AuthorityKey::module(METADATA_AUTHORITY);
            ...
            let royalty_authority = AuthorityKey::module(ROYALTY_AUTHORITY);
...
impl AccessRulesNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_SET_AUTHORITY_RULE_IDENT => {
                ...
                let rtn =
                    Self::set_authority_access_rule(input.object_key, input.authority_key, input.rule, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ACCESS_RULES_SET_AUTHORITY_MUTABILITY_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        Self::set_authority_access_rule_and_mutability_internal(
            object_key, authority_key, Some(rule), None, api)
    }

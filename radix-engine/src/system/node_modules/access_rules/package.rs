...
impl CycleCheck<String> for AccessRulesConfig {
    fn get_references(&self, authority: &String) -> Result<IndexSet<String>, CycleCheckError<String>> {
...
impl CycleCheck<String> for AuthorityRules {
    fn get_references(&self, authority: &String) -> Result<IndexSet<String>, CycleCheckError<String>> {
...
impl AccessRulesConfig {
    ...
    fn create(
    ...
    ) -> Result<Self, CycleCheckError<String>> {
        ...
        if !authority_rules.rules.contains_key(METADATA_AUTHORITY) {
            authority_rules.set_rule(METADATA_AUTHORITY, rule!(require("owner")), rule!(require("owner")));
        }
        if !authority_rules.rules.contains_key(ROYALTY_AUTHORITY) {
            authority_rules.set_rule(ROYALTY_AUTHORITY, rule!(require("owner")), rule!(require("owner")));
        }
    ...
    pub fn set_authority_access_rule<E: Into<AccessRule>>(
        ...
        authority: String,
        access_rule_entry: E,
    ) -> Result<(), CycleCheckError<String>> {
        self.rules.insert(authority.clone(), access_rule_entry.into());

        let mut visited = index_set_new();
        let mut stack = index_set_new();
        self.cycle_check(&authority, &mut visited, &mut stack)?;

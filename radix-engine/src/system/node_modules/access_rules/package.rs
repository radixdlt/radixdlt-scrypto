...
impl From<BTreeMap<String, BTreeMap<String, AccessRule>>> for FunctionAccessRulesSubstate {
    fn from(value: BTreeMap<String, BTreeMap<String, AccessRule>>) -> Self {
        let mut access_rules = BTreeMap::new();
        for (blueprint, function_rules) in value {
            for (ident, rule) in function_rules {
...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageSetup {
        ...
        collections.push(BlueprintCollectionSchema::KeyValueStore(
            BlueprintKeyValueStoreSchema {
                key: TypeRef::Blueprint(aggregator.add_child_type_and_descendents::<RoleKey>()),
        ...
        collections.push(BlueprintCollectionSchema::KeyValueStore(
            BlueprintKeyValueStoreSchema {
                key: TypeRef::Blueprint(aggregator.add_child_type_and_descendents::<RoleKey>()),
                value: TypeRef::Blueprint(
                    aggregator.add_child_type_and_descendents::<(RoleList, bool)>(),
                ),
                can_own: false,
            },
        ));
        ...
        functions.insert(
            ...
            FunctionSchema {
                ...
                export: FeaturedSchema::normal(ACCESS_RULES_CREATE_IDENT),
            },
        );
        functions.insert(
            ...
            FunctionSchema {
                ...
                export: FeaturedSchema::normal(ACCESS_RULES_UPDATE_ROLE_IDENT),
            },
        );
        functions.insert(
            ...
            FunctionSchema {
                ...
                export: FeaturedSchema::normal(ACCESS_RULES_GET_ROLE_IDENT),
            },
        );
        ...
        let blueprints = btreemap!(
            ACCESS_RULES_BLUEPRINT.to_string() => BlueprintSetup {
        ...
        Y: KernelSubstateApi<L>,
    {
        ...
        let substate: SubstateWrapper<Option<(RoleList, bool)>> =
            api.kernel_read_substate(handle)?.as_typed().unwrap();
        api.kernel_drop_lock(handle)?;
        let (role_list, mutable) = substate.value.unwrap_or_else(|| (RoleList::none(), false));
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (role_key, entry) in roles.roles {
            role_entries.insert(
                ...
                scrypto_encode(&entry.rule).unwrap(),
            );
            mutability_entries.insert(
                scrypto_encode(&role_key).unwrap(),
                scrypto_encode(&(entry.mutable, entry.mutable_mutable)).unwrap(),
            );
        }
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if let Some(mutability) = &mutability {
            ...
            let substate: Option<(RoleList, bool)> = api.key_value_entry_get_typed(handle)?;
            if substate.is_none() {
                return Err(RuntimeError::ApplicationError(
                    ApplicationError::AccessRulesError(AccessRulesError::RoleDoesNotExist(
                        role_key,
            ...
            // TODO: Use KV mutability freezing instead of separate flag
            api.key_value_entry_set_typed(handle, mutability.clone())?;
            api.key_value_entry_release(handle)?;
        }

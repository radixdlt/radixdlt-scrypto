...
use radix_engine_interface::api::types::{GlobalAddress, PackageOffset, RENodeId, SubstateOffset};
...
impl AccessRulesNativePackage {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        // Abi checks
        {
            ...
            for (key, _) in input.access_rules.get_all_method_auth() {
                if let AccessRuleKey { node_module_id: NodeModuleId::SELF, method_ident, } = key {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        if matches!(
            input.key,
            AccessRuleKey { node_module_id: NodeModuleId::AccessRules, .. }
        ) || matches!(
            input.key,
            AccessRuleKey { node_module_id: NodeModuleId::AccessRules1, .. }
        ) {
            return Err(RuntimeError::ApplicationError(
        ...
        access_rules.set_method_access_rule(input.key, input.rule);
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        access_rules.set_group_access_rule(input.name, input.rule);
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        access_rules.set_mutability(input.key, input.mutability);
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        access_rules.set_group_mutability(input.name, input.mutability);

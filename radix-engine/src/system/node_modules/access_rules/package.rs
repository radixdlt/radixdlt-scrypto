...
use crate::system::system_modules::auth::{convert_contextless, MethodAuthorization};
...
impl AccessRulesNativePackage {
    ...
    pub fn method_mutability_authorization<Y, L: Default>(
        receiver: &NodeId,
        object_key: ObjectKey,
    ...
    where
        Y: KernelNodeApi + KernelSubstateApi<L>,
    {
        let authorization = if method_key.module_id.eq(&ObjectModuleId::AccessRules) {
    ...
    {
        let handle =
            api.actor_lock_field(OBJECT_HANDLE_SELF, AccessRulesField::AccessRules.into(), LockFlags::MUTABLE)?;

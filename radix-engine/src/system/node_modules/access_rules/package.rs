...
use radix_engine_interface::api::{ClientApi, ClientObjectApi, ObjectModuleId, OBJECT_HANDLE_SELF, KVEntry};
...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            ...
            FunctionSchemaInit {
                ...
                input: TypeRef::Static(
                    aggregator
                        .add_child_type_and_descendents::<AccessRulesSetAndLockOwnerRoleInput>(),
                ),
                output: TypeRef::Static(
                    aggregator
                        .add_child_type_and_descendents::<AccessRulesSetAndLockOwnerRoleOutput>(),
                ),
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (module, roles) in roles {
            for (role_key, (rule, locked)) in roles.roles {
                if Self::is_reserved_role_key(&role_key) {
                    return Err(RuntimeError::ApplicationError(
                ...
                let kv_entry = KVEntry {
                    value: Some(scrypto_encode(&rule).unwrap()),
                    locked
                };

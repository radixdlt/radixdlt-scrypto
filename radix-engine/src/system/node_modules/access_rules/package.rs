...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageSetup {
        ...
        fields.push(FieldSchema::normal(aggregator.add_child_type_and_descendents::<MethodAccessRulesSubstate>()));
    ...
    pub(crate) fn create<Y>(role_definitions: Roles, api: &mut Y) -> Result<Own, RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        if role_definitions
        ...
        for (role_key, entry) in role_definitions.roles {
            roles.insert(role_key.clone(), entry.rule);
            protected_roles.insert(role_key, (entry.mutable, entry.mutable_mutable));
        }

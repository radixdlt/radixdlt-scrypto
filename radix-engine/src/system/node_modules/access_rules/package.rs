...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        fields.push(FieldSchema::static_field(
            aggregator.add_child_type_and_descendents::<OwnerRole>(),
        ));
        ...
        let blueprints = btreemap!(
            ACCESS_RULES_BLUEPRINT.to_string() => BlueprintDefinitionInit {
                ...
                auth_config: AuthConfig {
                    ...
                    method_auth: MethodAuthTemplate::Static(btreemap!()), // Mocked
                },
            }
        );
    ...
    fn resolve_update_role_method_permission<Y, L: Default>(
        receiver: &NodeId,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let mut updater_entries = BTreeMap::new();

        for (module, roles) in roles {
            for (role_key, (entry, freeze)) in roles.roles {
                let module_role_key = ModuleRoleKey::new(module, role_key);

                role_entries.insert(
                    scrypto_encode(&module_role_key).unwrap(),
                    (scrypto_encode(&entry.rule).unwrap(), freeze),
                );
                updater_entries.insert(
                    scrypto_encode(&module_role_key).unwrap(),
                    (scrypto_encode(&entry.updaters).unwrap(), true),
                );
            }
        }

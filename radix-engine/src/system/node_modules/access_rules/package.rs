...
use crate::system::node_modules::access_rules::UpdateOwnerRoleEvent;
...
use radix_engine_interface::api::{ClientApi, ClientObjectApi, ObjectModuleId, OBJECT_HANDLE_SELF};
use radix_engine_interface::blueprints::package::{
    AuthConfig, BlueprintDefinitionInit, BlueprintType, BlueprintVersionKey, MethodAuthTemplate,
    PackageDefinition, StaticRoles,
};
...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            ACCESS_RULES_UPDATE_OWNER_ROLE_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(schema::ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesUpdateOwnerRoleInput>(),
                ),
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesUpdateOwnerRoleOutput>(),
                ),
                export: ACCESS_RULES_UPDATE_OWNER_ROLE_IDENT.to_string(),
            },
        );
        functions.insert(
        ...
        functions.insert(
            ACCESS_RULES_LOCK_ROLE_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(schema::ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesLockRoleInput>(),
                ),
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesSetAndLockRoleOutput>(),
                ),
                export: ACCESS_RULES_LOCK_ROLE_IDENT.to_string(),
            },
        );
        functions.insert(
            ACCESS_RULES_SET_AND_LOCK_ROLE_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(schema::ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesSetAndLockRoleInput>(),
                ),
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesSetAndLockRoleOutput>(),
                ),
                export: ACCESS_RULES_SET_AND_LOCK_ROLE_IDENT.to_string(),
            },
        );
        functions.insert(
        ...
        let blueprints = btreemap!(
            ACCESS_RULES_BLUEPRINT.to_string() => BlueprintDefinitionInit {
                ...
                auth_config: AuthConfig {
                    ...
                    method_auth: MethodAuthTemplate::Static(StaticRoles::default()), // Mocked
                },
            }
        );
    ...
    pub fn authorization<Y: KernelApi<SystemConfig<V>>, V: SystemCallbackObject>(
        ...
        let permission = match ident {
            ...
            ACCESS_RULES_LOCK_ROLE_IDENT => {
                let input: AccessRulesLockRoleInput = input.as_typed().map_err(|e| {
                    RuntimeError::ApplicationError(ApplicationError::InputDecodeError(e))
                })?;
                let role_list = Self::resolve_update_role_method_permission(
                    node_id,
            ...
            ACCESS_RULES_SET_AND_LOCK_ROLE_IDENT => {
                let input: AccessRulesSetAndLockRoleInput = input.as_typed().map_err(|e| {
                    RuntimeError::ApplicationError(ApplicationError::InputDecodeError(e))
                })?;
                let role_list = Self::resolve_update_role_method_permission(
                    node_id,
            ...
            ACCESS_RULES_UPDATE_OWNER_ROLE_IDENT => {
                let input: AccessRulesUpdateOwnerRoleInput = input.as_typed().map_err(|e| {
                    RuntimeError::ApplicationError(ApplicationError::InputDecodeError(e))
                })?;
                Self::resolve_update_owner_role_method_permission(node_id, &input, api)?
            }
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_UPDATE_OWNER_ROLE_IDENT => {
                ...
                let rtn = Self::update_owner_role(input.rule, input.freeze, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ACCESS_RULES_SET_ROLE_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;
            ...
            ACCESS_RULES_LOCK_ROLE_IDENT => {
                ...
                let rtn = Self::lock_role(input.module, input.role_key, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ACCESS_RULES_SET_AND_LOCK_ROLE_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;
                ...
                let rtn = Self::set_and_lock_role(input.module, input.role_key, input.rule, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
    ...
    >(
        receiver: &NodeId,
        _input: &AccessRulesUpdateOwnerRoleInput,
        api: &mut SystemService<Y, V>,
    ) -> Result<ResolvedPermission, RuntimeError> {
        ...
        let rule = match owner_role_substate.owner_role_entry.updater {
            OwnerRoleUpdater::None => AccessRule::DenyAll,
            OwnerRoleUpdater::Owner => owner_role_substate.owner_role_entry.rule,
    ...
    >(
        ...
        match auth_template {
            ...
            MethodAuthTemplate::StaticUseOuterRoles(..) => Ok(RoleList::none()),
            MethodAuthTemplate::Static(roles) => match roles.roles.get(&role_key) {
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (module, roles) in roles {
            for (role_key, (rule, freeze)) in roles.roles {
                if Self::is_reserved_role_key(&role_key) {
                    return Err(RuntimeError::ApplicationError(
                ...
                role_entries.insert(
                    ...
                    (scrypto_encode(&rule).unwrap(), freeze),
                );
            }
        }

        let owner_role_substate = OwnerRoleSubstate {
            owner_role_entry: owner_role.to_entry(),
        };
        ...
        let component_id = api.new_object(
            ...
            vec![],
            None,
            vec![scrypto_encode(&owner_role_substate).unwrap()],
            btreemap!(
                0u8 => role_entries,
            ),
        )?;
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        let handle = api.actor_lock_field(OBJECT_HANDLE_SELF, 0u8, LockFlags::MUTABLE)?;

        let mut owner_role: OwnerRoleSubstate = api.field_lock_read_typed(handle)?;
        if let Some(update) = &rule {
            owner_role.owner_role_entry.rule = update.clone();
        }
        if freeze {
            owner_role.owner_role_entry.updater = OwnerRoleUpdater::None;
        }

        api.field_lock_write_typed(handle, owner_role)?;
        api.field_lock_release(handle)?;

        Runtime::emit_event(api, UpdateOwnerRoleEvent { rule, freeze })?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        Runtime::emit_event(api, LockRoleEvent { role_key })?;

        Ok(())
    }

    fn set_and_lock_role<Y>(
        module: ObjectModuleId,
        role_key: RoleKey,
        rule: AccessRule,
        api: &mut Y,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        Runtime::emit_event(api, SetAndLockRoleEvent { role_key, rule })?;

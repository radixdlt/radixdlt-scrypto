...
pub struct NodeAuthorityRules {
    pub roles: BTreeMap<AuthorityKey, AccessRule>,
    
    pub protected_methods: BTreeMap<MethodKey, Vec<String>>,
    pub protected_roles: BTreeMap<AuthorityKey, Vec<String>>,
}
...
impl CycleCheck<AuthorityKey> for Roles {
    fn get_references(
        &self,
        authority: &AuthorityKey,
    ) -> Result<IndexSet<AuthorityKey>, CycleCheckError<AuthorityKey>> {
...
impl NodeAuthorityRules {
    pub fn new() -> Self {
        Self {
            protected_methods: BTreeMap::new(),
            roles: BTreeMap::new(),
    ...
    fn create(
        ...
        if !inner_blueprint {
            ...
            if !authority_rules
                .rules
                .contains_key(&AuthorityKey::new("owner"))
            {
                authority_rules.define_role("owner", AccessRule::DenyAll, vec![])
            }

            // Metadata
            {
                // Always public
                protected_module_methods.remove(&MethodKey::new(
        ...
        Ok(Self {
            protected_methods: protected_module_methods,
            roles: rules,
            protected_roles,
        })
    }

    pub fn get_authority_mutability(&self, key: &AuthorityKey) -> Vec<String> {
    ...
    pub fn set_authority_mutability<S: ToString>(
        &mut self,
        authority_key: AuthorityKey,
        roles: &Vec<S>,
    ) -> Result<(), CycleCheckError<AuthorityKey>> {
        self.protected_roles
            .insert(authority_key.clone(), roles.iter().map(|s| s.to_string()).collect());
        Ok(())
    }
}
...
impl AccessRulesNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ACCESS_RULES_CREATE_IDENT => {
                ...
                let rtn = Self::create(
                    input.protected_module_methods,
    ...
    {
        let access_rules =
            NodeAuthorityRules::create(protected_module_methods, false, authority_rules).map_err(
                |e| {
    ...
    pub(crate) fn set_authority_mutability<Y, S: ToString>(
        object_key: ObjectKey,
        authority_key: AuthorityKey,
        mutability: Vec<S>,
        api: &mut Y,
    ...
    pub(crate) fn set_authority_access_rule_and_mutability<Y, S: ToString>(
        object_key: ObjectKey,
        authority_key: AuthorityKey,
        rule: AccessRule,
        mutability: Vec<S>,
    ...
    fn set_authority_access_rule_and_mutability_internal<Y, S: ToString>(
        object_key: ObjectKey,
        authority_key: AuthorityKey,
        rule: Option<AccessRule>,
        mutability: Option<Vec<S>>,
        api: &mut Y,
    ) -> Result<(), RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        ...
        if let Some(mutability) = &mutability {
            access_rules
                .set_authority_mutability(authority_key.clone(), mutability)
                .map_err(|cycle| {
                    RuntimeError::ModuleError(ModuleError::AuthError(AuthError::CycleCheckError(
                        cycle,
                    )))
        ...
        if let Some(mutability) = mutability {
            Runtime::emit_event(
                ...
                SetMutabilityEvent {
                    authority_key: authority_key.clone(),
                    mutability: mutability.into_iter().map(|s| s.to_string()).collect(),
                },
            )?;
        }

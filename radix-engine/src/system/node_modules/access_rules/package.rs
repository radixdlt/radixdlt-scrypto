...
pub enum AccessRulesError {
    ...
    RoleDoesNotExist(ObjectModuleId, RoleKey),
...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            ...
            FunctionSchemaInit {
                ...
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesUpdateRoleOutput>(),
                ),
    ...
    pub fn authorization<Y: KernelApi<SystemConfig<V>>, V: SystemCallbackObject>(
        ...
        let permission = match ident {
            ACCESS_RULES_UPDATE_ROLE_IDENT => {
                ...
                let role_list = Self::resolve_update_role_method_permission(node_id, &input, api)?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_UPDATE_ROLE_IDENT => {
                ...
                let rtn =
                    Self::update_role(input.module, input.role_key, input.rule, input.freeze, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
    ...
    >(
        ...
        let permission = match owner_role_substate.owner_role_entry.updater {
            OwnerRoleUpdater::None => ResolvedPermission::AccessRule(AccessRule::DenyAll),
            OwnerRoleUpdater::Owner => ResolvedPermission::AccessRule(owner_role_substate.owner_role_entry.rule),
        };
    ...
    >(
        ...
        if Self::is_reserved_role_key(&input.role_key) {
            return Ok(RoleList::none());
        }

        let blueprint_id = match input.module {
            ObjectModuleId::Main => {
        ...
        match auth_template {
            MethodAuthTemplate::NoAuth => Ok(RoleList::none()),
            MethodAuthTemplate::StaticUseOuterRoles(..) => Ok(RoleList::none()),
            MethodAuthTemplate::Static(roles) => match roles.roles.get(&input.role_key) {
        ...
        Y: ClientApi<RuntimeError>,
    {
        if roles.contains_key(&ObjectModuleId::AccessRules) {
            return Err(RuntimeError::ApplicationError(ApplicationError::AccessRulesError(AccessRulesError::UsedReservedSpace)));
        }

...
use radix_engine_interface::api::{ClientApi, OBJECT_HANDLE_SELF, ObjectModuleId};
...
impl CycleCheck<AuthorityKey> for NodeAuthorityRules {
    fn get_references(
        ...
        let references = if let Some(access_rule) = self.rules.get(authority) {
            access_rule.get_referenced_authorities()
        } else {
        ...
        match authority {
            AuthorityKey::Main(..) => {
                let references = references
                    .into_iter()
                    .map(|rule| AuthorityKey::from_access_rule(rule))
                    .collect();
                Ok(references)
            }
        }
    ...
    fn get_references(
        ...
        let references =
            if let Some((access, _mutability)) = self.rules.get(authority) {
                access.get_referenced_authorities()
            } else {
                return Err(CycleCheckError::MissingAuthority(authority.clone()));
            };
...
impl NodeAuthorityRules {
    ...
    fn create(
        mut protected_module_methods: BTreeMap<MethodKey, Vec<String>>,
        ...
        if !inner_blueprint {
            ...
            if !authority_rules.rules.contains_key(&AuthorityKey::main("owner")) {
                authority_rules.set_owner_authority(AccessRule::DenyAll, AccessRule::DenyAll)
            }

            // Metadata
            {
                // Always public
                protected_module_methods.remove(&MethodKey::new(ObjectModuleId::Metadata, METADATA_GET_IDENT));
                let metadata_set = MethodKey::new(ObjectModuleId::Metadata, METADATA_SET_IDENT);
                if !protected_module_methods.contains_key(&metadata_set) {
                    protected_module_methods.insert(metadata_set, vec![]);
                }
            }

            {
                let royalty_set = MethodKey::new(ObjectModuleId::Royalty, COMPONENT_ROYALTY_SET_ROYALTY_CONFIG_IDENT);
                if !protected_module_methods.contains_key(&royalty_set) {
                    let mut authorities = vec![];
                    if authority_rules.rules.contains_key(&AuthorityKey::main("owner")) {
                        authorities.push("owner".to_string());
                    }
                    protected_module_methods.insert(royalty_set, authorities);
                }
                let royalty_claim = MethodKey::new(ObjectModuleId::Royalty, COMPONENT_ROYALTY_CLAIM_ROYALTY_IDENT);
                if !protected_module_methods.contains_key(&royalty_claim) {
                    let mut authorities = vec![];
                    if authority_rules.rules.contains_key(&AuthorityKey::main("owner")) {
                        authorities.push("owner".to_string());
                    }
                    protected_module_methods.insert(royalty_claim, authorities);
                }
            }
        }
        ...
        Ok(Self { protected_module_methods, rules, mutability })
    }
...
impl AccessRulesNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ACCESS_RULES_CREATE_IDENT => {
                ...
                let rtn = Self::create(input.protected_module_methods, input.authority_rules, input.inner_blueprint_rules, api)?;
    ...
    pub(crate) fn create<Y>(
        ...
        child_blueprint_rules: BTreeMap<String, AuthorityRules>,
        api: &mut Y,
    ) -> Result<Own, RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        let access_rules = NodeAuthorityRules::create(protected_module_methods, false, authority_rules).map_err(|e| {
        ...
        for (blueprint, authority_rules) in child_blueprint_rules {
            let inner_access_rules =
                NodeAuthorityRules::create(btreemap!(), true, authority_rules).map_err(|e| {
                    RuntimeError::ModuleError(ModuleError::AuthError(AuthError::CycleCheckError(e)))
                })?;

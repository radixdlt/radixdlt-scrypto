                ...
                for mutability_ref in mutability_references {
                    match mutability_ref {
                        ...
                        AuthorityRule::Custom(..) => {
                            return Err(CycleCheckError::OwnerAuthorityMutabilityHasInvalidReferences);
                        }
                    }
                }
                Ok(index_set_new())
...
impl CycleCheck<AuthorityKey> for AuthorityRules {
    fn get_references(
        ...
        let (references, mutability_references) = if let Some((access, mutability)) = self.rules.get(authority) {
            (
...
impl AccessRulesNativePackage {
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        let access_rules = NodeAuthorityRules::create(false, authority_rules)
            .map_err(|e| {

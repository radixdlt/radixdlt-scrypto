...
pub const PUBLIC_AUTHORITY: &str = "public";
...
impl AccessRulesConfig {
    ...
    fn get_references(&self, authority: &String) -> Result<IndexSet<String>, CycleCheckError> {
        ...
        if let Some(access_rule) = self.rules.get(authority) {
            ...
            return Err(CycleCheckError::MissingAuthority(authority.to_string()));
        }
    ...
    fn self_check_for_cycles(
        &self,
        authority: String,
        visited: &mut IndexSet<String>,
        stack: &mut IndexSet<String>,
    ) -> Result<(), CycleCheckError>{
        if stack.contains(&authority) {
            return Err(CycleCheckError::CycleFound(authority.clone()));
        }
        ...
        stack.insert(authority.clone());

        let referenced = self.get_references(&authority)?;

        for ref_authority in referenced {
            self.self_check_for_cycles(ref_authority, visited, stack)?;
        }

        stack.remove(&authority);

        Ok(())
    }

    fn check_for_cycles(
        authorities: &IndexMap<String, IndexSet<String>>,
        authority: String,
        visited: &mut IndexSet<String>,
        stack: &mut IndexSet<String>,
    ) -> Result<(), CycleCheckError> {
        ...
        let referenced = authorities.get(&authority).ok_or_else(|| {
            CycleCheckError::MissingAuthority(authority.clone())
        })?;

        for ref_authority in referenced {
            Self::check_for_cycles(authorities, ref_authority.clone(), visited, stack)?;
        }
    ...
    fn create(
    ...
    ) -> Result<Self, CycleCheckError> {
        // TODO: virtualize this state
        if !authority_rules.rules.contains_key("owner") {
            authority_rules.set_rule("owner", AccessRule::DenyAll, AccessRule::DenyAll)
        }
        ...
        let mut authority_forest = index_map_new();
        ...
        for (authority, (rule, mutability_rule)) in authority_rules.rules {
            ...
            authority_forest.insert(authority.clone(), referenced);
        ...
        for (authority, _references) in &authority_forest {
            Self::check_for_cycles(&authority_forest, authority.clone(), &mut visited, &mut stack)?;
        }
    ...
    pub fn set_authority_access_rule<E: Into<AccessRule>>(
        ...
        self.rules.insert(authority.clone(), access_rule_entry.into());

        let mut visited = index_set_new();
        let mut stack = index_set_new();
        self.self_check_for_cycles(authority, &mut visited, &mut stack)?;
    ...
    pub fn set_authority_mutability<M: Into<AccessRule>>(
        ...
        self.mutability.insert(authority.clone(), access_rule.into());

        let mut visited = index_set_new();
        let mut stack = index_set_new();
        self.self_check_for_cycles(authority.clone(), &mut visited, &mut stack)?;
...
impl AccessRulesNativePackage {
        ...
        Y: KernelSubstateApi<L>,
    {
        ...
        let access_rule = {
            ...
            access_rules_substate.authority_mutability_access_rule(&object_key, name)
        };
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        // Predefined rules
        authority_rules.set_rule(PUBLIC_AUTHORITY, AccessRule::AllowAll, AccessRule::DenyAll);

        let access_rules =
            AccessRulesConfig::create(method_authorities, authority_rules).map_err(|e| {
                RuntimeError::ModuleError(ModuleError::AuthError(
                    AuthError::CycleCheckError(e),
                ))
            })?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if let Some(rule) = &rule {
            access_rules
                ...
                .map_err(|cycle| {
                    RuntimeError::ModuleError(ModuleError::AuthError(
                        AuthError::CycleCheckError(cycle),
                    ))
                })?;
        }

...
impl CycleCheck<RoleKey> for NodeAuthorizationRules {
    fn get_references(
        &self,
        authority: &RoleKey,
...
impl AccessRulesNativePackage {
    ...
    {
        let access_rules =
            NodeAuthorizationRules::create(method_permissions, false).map_err(
                |e| {
        ...
        for (blueprint, protected_methods) in inner_blueprint_rules {
            let inner_access_rules = NodeAuthorizationRules::create(
                protected_methods,
                true,
            )
            .map_err(|e| {
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if let Some(mutability) = mutability {
            access_rules_substate.protected_roles.insert(role_key.clone(), mutability.clone());
            Runtime::emit_event(
                api,
                SetRoleMutabilityEvent {
                    role_key: role_key,

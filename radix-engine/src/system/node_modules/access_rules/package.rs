...
impl AccessRulesNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            ACCESS_RULES_SET_AND_LOCK_OWNER_ROLE_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesSetAndLockOwnerRoleInput>(),
                ),
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<AccessRulesSetAndLockOwnerRoleOutput>(),
                ),
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_SET_AND_LOCK_OWNER_ROLE_IDENT => {
                ...
                let input: AccessRulesSetAndLockOwnerRoleInput = input.as_typed().map_err(|e| {
                    RuntimeError::ApplicationError(ApplicationError::InputDecodeError(e))
                })?;

                let rtn = Self::set_and_lock_owner_role(input.rule, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ACCESS_RULES_SET_ROLE_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;
    ...
    fn set_and_lock_owner_role<Y>(rule: AccessRule, api: &mut Y) -> Result<(), RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        let handle = api.actor_lock_field(OBJECT_HANDLE_SELF, 0u8, LockFlags::MUTABLE)?;
        ...
        Runtime::emit_event(api, SetAndLockOwnerRoleEvent { rule })?;

...
impl NodeAuthorityRules {
    pub fn get_rule(&self, authority: &str) -> Option<&AccessRule> {
        self.rules.get(&AuthorityKey::new(authority))
    }
}
...
impl CycleCheck<AuthorityKey> for NodeAuthorityRules {
    fn get_references(
        ...
        let references = references.into_iter()
            .map(|s| AuthorityKey::new(s.as_str()))
            .collect();
...
impl CycleCheck<AuthorityKey> for AuthorityRules {
    fn get_references(
        ...
        let referenced = if let Some((access, mutability)) = self.rules.get(&authority.authority) {
...
impl NodeAuthorityRules {
    ...
    fn create(
        ...
        for authority in authority_rules.rules.keys() {
            authority_rules.cycle_check(&AuthorityKey::new(authority.as_str()), &mut visited, &mut stack)?;
        }
        ...
        for (authority, (rule, mutability_rule)) in authority_rules.rules {
            rules.insert(AuthorityKey::new(authority.as_str()), rule);
            mutability.insert(AuthorityKey::new(authority.as_str()), mutability_rule);
        }
...
impl AccessRulesNativePackage {
        ...
        let access_rule = match ident {
            ACCESS_RULES_SET_AUTHORITY_RULE_AND_MUTABILITY_IDENT => {
                ...
                Self::authority_mutability_authorization(
                    node_id,
                    input.object_key,
                    &AuthorityKey::new(input.name.as_str()),
                    api,
                )?
            }
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_SET_AUTHORITY_RULE_IDENT => {
                ...
                let rtn =
                    Self::set_authority_access_rule(input.object_key, input.name, input.rule, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ACCESS_RULES_SET_AUTHORITY_MUTABILITY_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        Self::set_authority_access_rule_and_mutability_internal(
            object_key, AuthorityKey::new(name.as_str()), Some(rule), None, api)
    }

...
use crate::system::node_modules::access_rules::{SetMethodMutabilityEvent, SetMethodPermissionEvent};
...
impl AccessRulesNativePackage {
    pub fn schema() -> PackageSchema {
        ...
        functions.insert(
            ACCESS_RULES_SET_METHOD_PERMISSION_AND_MUTABILITY_IDENT.to_string(),
            FunctionSchema {
                receiver: Some(schema::ReceiverInfo::normal_ref_mut()),
                input: aggregator
                    .add_child_type_and_descendents::<AccessRulesSetMethodPermissionAndMutabilityInput>(),
                output: aggregator
                    .add_child_type_and_descendents::<AccessRulesSetMethodPermissionAndMutabilityOutput>(),
                export_name: ACCESS_RULES_SET_METHOD_PERMISSION_AND_MUTABILITY_IDENT.to_string(),
            },
        );
        functions.insert(
        ...
        let event_schema = event_schema! {
            aggregator,
            [
                SetMethodPermissionEvent,
                SetMethodMutabilityEvent,
                DefineRoleEvent,
    ...
    pub fn authorization<Y: KernelSubstateApi<L>, L: Default>(
        ...
        let roles = match ident {
            ACCESS_RULES_SET_METHOD_PERMISSION_AND_MUTABILITY_IDENT => {
                let input: AccessRulesSetMethodPermissionAndMutabilityInput =
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        Self::set_method_permissions_and_mutability_internal(
            object_key,
            method_key,
            Some(permission),
            Some(mutability),
    ...
    fn set_method_permissions_and_mutability_internal<Y>(
        object_key: ObjectKey,
        method_key: MethodKey,

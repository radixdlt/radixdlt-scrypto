...
use super::{DefineRoleEvent, SetRoleMutabilityEvent};
...
impl MethodAccessRulesSubstate {
    pub fn role_mutability_list(&self, key: &RoleKey) -> RoleList {
...
impl NodeAuthorizationRules {
    ...
    pub fn set_method_permission(&mut self, method_key: MethodKey, permission: MethodPermission) {
        self.method_permissions
            .insert(method_key.clone(), permission);
    }

    pub fn set_method_mutability(&mut self, method_key: MethodKey, mutability: RoleList) {
...
impl AccessRulesNativePackage {
    ...
    pub fn authorization<Y: KernelSubstateApi<L>, L: Default>(
        ...
        let permission = match ident {
            ACCESS_RULES_UPDATE_METHOD_IDENT => {
                let input: AccessRulesUpdateMethod = input.as_typed().map_err(|e| {
            ...
            ACCESS_RULES_DEFINE_ROLE_IDENT => {
                ...
                Self::role_mutability_authorization(node_id, &input.role_key, api)?
            }
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            ACCESS_RULES_DEFINE_ROLE_IDENT => {
                ...
                let rtn = Self::define_role(input.role_key, input.rule, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
            ...
            ACCESS_RULES_SET_ROLE_MUTABILITY_IDENT => {
                ...
                let rtn = Self::set_role_mutability(input.role_key, input.mutability, api)?;
                Ok(IndexedScryptoValue::from_typed(&rtn))
            }
        ...
        Y: KernelSubstateApi<L>,
    {
        ...
        let access_rule = {
            let access_rules_substate: MethodAccessRulesSubstate =
                api.kernel_read_substate(handle)?.as_typed().unwrap();
            access_rules_substate.role_mutability_list(authority_key)
        };
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (authority, (rule, mutability_rule)) in role_definitions.rules {
            roles.insert(authority.clone(), rule);
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if let Some(permission) = &permission {
            access_rules.set_method_permission(method_key.clone(), permission.clone());
        }
        if let Some(mutability) = &mutability {
            access_rules.set_method_mutability(method_key.clone(), mutability.clone());
        }
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        Self::update_role_and_mutability_internal(
            authority_key,
            Some(rule),
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        Self::update_role_and_mutability_internal(authority_key, None, Some(mutability), api)
    }

    fn update_role_and_mutability_internal<Y>(
        role_key: RoleKey,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        if !access_rules_substate.roles.contains_key(&role_key) {
            return Err(RuntimeError::ApplicationError(ApplicationError::AccessRulesError(AccessRulesError::RoleDoesNotExist(role_key))));
        }

...
impl RoyaltyNativePackage {
    pub fn function_access_rules() -> BTreeMap<FnKey, AccessRule> {
        ...
        access_rules.insert(
            FnKey::new(COMPONENT_ROYALTY_BLUEPRINT.to_string(), COMPONENT_ROYALTY_CREATE_IDENT.to_string()),
            rule!(allow_all),
        );
        access_rules
    }
...
impl ComponentRoyaltyBlueprint {
    pub(crate) fn create<Y>(
        ...
        where
            Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        let input: ComponentRoyaltyCreateInput = scrypto_decode(&scrypto_encode(&input).unwrap())
        ...
        api.kernel_create_node(
            node_id,
                RENodeInit::ComponentRoyalty(ComponentRoyaltyConfigSubstate {

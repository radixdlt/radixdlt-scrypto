...
impl ComponentRoyaltyBlueprint {
    fn verify_royalty_amounts<'a, I: Iterator<Item = &'a RoyaltyAmount>, Y>(
        royalty_amounts: I,
        api: &mut Y,
    ) -> Result<(), RuntimeError>
                ...
                RoyaltyAmount::Xrd(xrd_amount) => {
                    if xrd_amount.gt(&max_royalty_in_xrd) {
                        return Err(
                            ...
                            max: royalty_amount.clone(),
                            actual: RoyaltyAmount::Xrd(xrd_amount.clone()),
                        })));
                    }
                },
                RoyaltyAmount::Usd(usd_amount) => {
                    if usd_amount.gt(&max_royalty_in_usd) {
                        return Err(
                            ...
                            ComponentRoyaltyError::RoyaltyAmountIsGreaterThanAllowed {
                            max: royalty_amount.clone(),
                            actual: RoyaltyAmount::Usd(usd_amount.clone()),
                        })));
                    }
                }
            }
        ...
        if let ComponentRoyaltyConfig::Enabled(royalty_amounts) = royalty_config {
            ...
            for (method, (amount, locked)) in royalty_amounts {
                royalty_config_entries.insert(
                    ...
                    (scrypto_encode(&amount).unwrap(), locked),
                );
            }
    ...
    {
        Self::verify_royalty_amounts(
            vec![amount.clone()].iter(),
            api,
        )?;

        let accumulator_handle = api.actor_lock_field(
            OBJECT_HANDLE_SELF,
    ...
    {
        let accumulator_handle = api.kernel_lock_substate(
            receiver,
        ...
        let component_royalty: ComponentRoyaltySubstate = api
            ...
            .unwrap();
        if !component_royalty.enabled {
            api.kernel_drop_lock(accumulator_handle)?;
            return Ok(());
        }

...
use crate::system::kernel_modules::auth::method_authorization::{
    MethodAuthorization, MethodAuthorizationError,
};
use crate::types::*;
            ...
            auth_zone_stack.check_auth(false, authorization).map_err(
                |(authorization, error)| {
                    RuntimeError::ApplicationError(ApplicationError::AccessRulesChainError(
                        AccessRulesChainError::UnauthorizedSetMethodAccessRules(self.key.clone(), authorization, error),
                    ))
                },
            )?;
        }
        ...
        // Manual Auth
        {
            ...
            auth_zone_substate
                ...
                .map_err(|(authorization, error)| {
                    RuntimeError::ApplicationError(ApplicationError::AccessRulesChainError(
                        AccessRulesChainError::UnauthorizedSetGroupAccessRules(self.name.clone(), authorization, error),
                    ))
                })?;
        }

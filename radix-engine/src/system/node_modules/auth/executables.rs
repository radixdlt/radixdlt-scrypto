...
impl AccessRulesNativePackage {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        // Abi checks
        {
            ...
            for (key, _) in input.access_rules.get_all_method_auth() {
                if let AccessRuleKey::ScryptoMethod(NodeModuleId::SELF, func_name) = key {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        if matches!(
            input.key,
            AccessRuleKey::ScryptoMethod(NodeModuleId::AccessRules, ..)
        ) || matches!(
            input.key,
            AccessRuleKey::ScryptoMethod(NodeModuleId::AccessRules1, ..)
        ) {
            return Err(RuntimeError::ApplicationError(

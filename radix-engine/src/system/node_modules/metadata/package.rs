...
use radix_engine_interface::schema::{BlueprintSchema, FunctionSchema, KeyValueStoreSchema, PackageSchema};
...
impl MetadataNativePackage {
    pub fn schema() -> PackageSchema {
        ...
        key_value_stores.push(KeyValueStoreSchema {
            key: aggregator.add_child_type_and_descendents::<String>(),
            value: aggregator.add_child_type_and_descendents::<Option<MetadataValueSubstate>>(),
            can_own: false,
        });
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        let node_id = api.new_object(METADATA_BLUEPRINT, vec![])?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (key, value) in data {
            // TODO: Use direct set kv store calls on owned object when supported by api
            let input = MetadataSetInput {
                key,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let handle =
            api.lock_key_value_entry(&scrypto_encode(&key).unwrap(), LockFlags::MUTABLE)?;

        let substate: MetadataValueSubstate = Option::Some(metadata.clone());
        api.key_value_entry_set(handle, scrypto_encode(&substate).unwrap())?;
        api.key_value_entry_lock_release(handle)?;
    ...
    {
        let handle =
            api.lock_key_value_entry(&scrypto_encode(&key).unwrap(), LockFlags::read_only())?;

        let data = api.key_value_entry_get(handle)?;
        let substate: MetadataValueSubstate = scrypto_decode(&data).unwrap();
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let value: MetadataValueSubstate = None;

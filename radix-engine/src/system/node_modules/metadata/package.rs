...
impl MetadataNativePackage {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            METADATA_SET_STRING_IDENT => {
                ...
                let input: MetadataSetStringInput = scrypto_decode(&scrypto_encode(&input).unwrap())
                    .map_err(|e| {
                        RuntimeError::InterpreterError(InterpreterError::ScryptoInputDecodeError(e))
                    })?;

                let rtn = Self::set_string(receiver, input.key, input.value, api)?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        for (key, value) in data {
            Self::set_string(RENodeId::Object(object_id), key, value, api)?;
        }

        Ok(Own::Object(object_id))
    }

    pub(crate) fn set_string<Y>(
        receiver: RENodeId,
        key: String,
        value: String,
        api: &mut Y,
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        Runtime::emit_event(api, SetEntryEvent { key, value })?;
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        let rtn = match substate {
            KeyValueStoreEntrySubstate::Some(.., ScryptoValue::String { value }) => Ok(value),
            KeyValueStoreEntrySubstate::Some(..) => Err(MetadataError::NotAString),

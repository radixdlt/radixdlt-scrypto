...
impl MetadataNativePackage {
    ...
    pub(crate) fn set<Y>(key: String, value: MetadataEntry, api: &mut Y) -> Result<(), RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
    ...
    where
        Y: ClientApi<RuntimeError>,
    {
        let cur_value: Option<MetadataEntry> = api.actor_remove_key_value_entry_typed(
            OBJECT_HANDLE_SELF,
            0u8,
            &scrypto_encode(&key).unwrap(),
        )?;

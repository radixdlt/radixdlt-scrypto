        ...
        Y: ClientApi<RuntimeError>,
    {
        for key in data.data.keys() {
            if key.len() > DEFAULT_MAX_METADATA_KEY_STRING_LEN {
                return Err(RuntimeError::ApplicationError(ApplicationError::MetadataError(MetadataPanicError::KeyStringExceedsMaxLength {
        ...
        for (key, entry) in data.data {
            ...
            let value = scrypto_encode(&entry.value).unwrap();
            if value.len() > DEFAULT_MAX_METADATA_VALUE_SBOR_LEN {
                return Err(RuntimeError::ApplicationError(ApplicationError::MetadataError(MetadataPanicError::ValueSborExceedsMaxLength {
                    max: DEFAULT_MAX_METADATA_VALUE_SBOR_LEN,
                    actual: value.len(),
                })));
            }
            init_kv_entries.insert(key, (value, entry.lock));
        }

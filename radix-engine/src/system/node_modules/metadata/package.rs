...
use crate::types::*;
use crate::{errors::*, event_schema, method_auth_template};
...
pub type MetadataValueSubstate = KeyValueEntrySubstate<MetadataValue>;
...
impl MetadataNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            METADATA_FREEZE_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(schema::ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<MetadataFreezeInput>(),
                ),
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<MetadataFreezeOutput>(),
                ),
                export: METADATA_FREEZE_IDENT.to_string(),
            },
        );
        functions.insert(
                ...
                auth_config: AuthConfig {
                    ...
                    method_auth: MethodAuthTemplate::Static(
                        method_auth_template!(
                            ...
                            METADATA_FREEZE_IDENT => [METADATA_SETTER_ROLE];
                            METADATA_REMOVE_IDENT => [METADATA_SETTER_ROLE];
                            METADATA_GET_IDENT => MethodPermission::Public;
                        ),
                    ),
                },
            }
        ...
        Y: ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            METADATA_FREEZE_IDENT => {
                api.consume_cost_units(FIXED_LOW_FEE, ClientCostingReason::RunNative)?;

                let input: MetadataFreezeInput = input.as_typed().map_err(|e| {
                    RuntimeError::ApplicationError(ApplicationError::InputDecodeError(e))
                })?;

                let rtn = Self::freeze(input.key, api)?;
    ...
    pub(crate) fn freeze<Y>(key: String, api: &mut Y) -> Result<(), RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        let handle = api.actor_lock_key_value_entry(

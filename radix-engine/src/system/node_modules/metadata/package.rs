...
pub fn validate_metadata_value(
    ...
    match value {
        ...
        MetadataValue::Url(url) => Ok(MetadataValue::Url(
            validate_url(url.0.as_str()).map(|x| Url(x.into()))?,
        )),
        MetadataValue::Origin(origin) => Ok(MetadataValue::Origin(
            validate_url(origin.0.as_str()).map(|x| Origin(x.into()))?,
        )),
        ...
        MetadataValue::UrlArray(urls) => Ok(MetadataValue::UrlArray(
            urls.into_iter()
                .map(|x| {
        ...
        MetadataValue::OriginArray(origins) => Ok(MetadataValue::OriginArray(
            origins
                .into_iter()
                .map(|x| {
                    validate_origin(x.0.as_str())
                        .map(|x| Origin(x.into()))
                        .map_err(|x| MetadataValidationError::OriginValidationError(x))
                })
                .collect::<Result<Vec<Origin>, MetadataValidationError>>()?,
        )),

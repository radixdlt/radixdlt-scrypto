...
impl MetadataNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        for key in data.data.keys() {
            if key.len() > DEFAULT_MAX_METADATA_KEY_LEN {
                return Err(RuntimeError::ApplicationError(ApplicationError::MetadataError(MetadataPanicError::KeyExceedsMaxLength {
                    max: DEFAULT_MAX_METADATA_KEY_LEN,
                    actual: key.len(),
                })))
            }
        }

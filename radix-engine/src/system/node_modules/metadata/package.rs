...
#[derive(Debug, Clone, Eq, PartialEq, ScryptoSbor)]
pub enum MetadataBackendValue {
    String(String)
}

impl MetadataBackendValue {
    fn to_value(self) -> MetadataValue {
        match self {
            MetadataBackendValue::String(value) => MetadataValue {
...
impl MetadataNativePackage {
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        match export_name {
            ...
            METADATA_GET_IDENT => {
                ...
                let rtn = Self::get(receiver, input.key, api)?;
    ...
    {
        // Schema check
        let backend_value = match value.type_id {
            METADATA_STRING_TYPE => match &value.value {
                ScryptoValue::String { value } => MetadataBackendValue::String(value.clone()),
                _ => {
                    return Err(RuntimeError::ApplicationError(
                        ApplicationError::MetadataError(MetadataPanicError::TypeDoesNotMatchSchema),
                    ))
                }
            },
            _ => {
                return Err(RuntimeError::ApplicationError(
                    ApplicationError::MetadataError(MetadataPanicError::UnknownValueType),
                ))
            }
        };
        ...
        let scrypto_value: ScryptoValue = scrypto_decode(&scrypto_encode(&backend_value).unwrap()).unwrap();
        ...
        Y: KernelNodeApi + KernelSubstateApi + ClientApi<RuntimeError>,
    {
        ...
        let rtn = match substate {
            KeyValueStoreEntrySubstate::Some(.., value) => {
                // TODO: Remove this encode/decode mess
                let metadata_value: MetadataBackendValue =
                    scrypto_decode(&scrypto_encode(&value).unwrap()).unwrap();
                Some(metadata_value.to_value())
            }

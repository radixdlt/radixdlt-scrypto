...
impl MetadataNativePackage {
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let node_id = api.new_object(METADATA_BLUEPRINT, None, vec![], btreemap!(0u8 => init_kv_entries))?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let handle =
            api.actor_lock_key_value_entry(0u8, &scrypto_encode(&key).unwrap(), LockFlags::MUTABLE)?;
    ...
    {
        let handle =
            api.actor_lock_key_value_entry(0u8, &scrypto_encode(&key).unwrap(), LockFlags::read_only())?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        let cur_value: Option<MetadataEntry> =
            api.actor_remove_key_value_entry_typed(0u8, &scrypto_encode(&key).unwrap())?;

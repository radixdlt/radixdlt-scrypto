...
impl MetadataNativePackage {
    ...
    > {
        ...
        for (key, entry) in data.data {
            ...
            let value = match entry.value {
                Some(metadata_value) => {
                    let value = scrypto_encode(&MetadataEntryEntryPayload::from_content_source(metadata_value)).unwrap();
        ...
        Ok((
            btreemap!(),
            btreemap!(MetadataCollection::EntryKeyValue.collection_index() => init_kv_entries),
        ))
    }
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let sbor_value = scrypto_encode(&MetadataEntryEntryPayload::from_content_source(value.clone())).unwrap();

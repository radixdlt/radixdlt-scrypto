...
pub enum RoleAssignmentError {
    ...
    ExceededMaxRuleDepth,
    ExceededMaxRuleNodes,
}
...
pub type RoleAssignmentRuleV1 = Rule;
...
impl RoleAssignmentNativePackage {
    ...
    pub fn verify_access_rule(access_rule: &Rule) -> Result<(), RoleAssignmentError> {
        pub struct RuleVerifier(usize);
        impl RuleVisitor for RuleVerifier {
            type Error = RoleAssignmentError;
            fn visit(&mut self, _node: &RuleNode, depth: usize) -> Result<(), Self::Error> {
                // This is to protect unbounded native stack useage during authorization
                if depth > MAX_ACCESS_RULE_DEPTH {
                    return Err(RoleAssignmentError::ExceededMaxRuleDepth);
                }

                self.0 += 1;

                if self.0 > MAX_ACCESS_RULE_NODES {
                    return Err(RoleAssignmentError::ExceededMaxRuleNodes);
                }
        ...
        access_rule.dfs_traverse_nodes(&mut RuleVerifier(0))
    }
    ...
    >(
        ...
        let rule = match owner_role.owner_role_entry.updater {
            OwnerRoleUpdater::None => Rule::DenyAll,
        ...
        Ok(ResolvedPermission::Rule(rule))
    }
    ...
    > {
        ...
        for (module, roles) in roles {
            for (role_key, role_def) in roles.data {
                ...
                let value = role_def.map(|rule| {
                    scrypto_encode(&RoleAssignmentRuleEntryPayload::from_content_source(rule))
                        .unwrap()
                });
        ...
        Ok((
            ...
            indexmap!(RoleAssignmentCollection::RuleKeyValue.collection_index() => role_entries),
        ))
    }
    ...
    fn set_owner_role<Y>(rule: Rule, api: &mut Y) -> Result<(), RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let handle = api.actor_open_key_value_entry(
            ACTOR_STATE_SELF,
            RoleAssignmentCollection::RuleKeyValue.collection_index(),
        ...
        api.key_value_entry_set_typed(
            handle,
            RoleAssignmentRuleEntryPayload::from_content_source(rule.clone()),
        )?;
    ...
    pub(crate) fn get_role<Y>(
        ...
        role_key: RoleKey,
        api: &mut Y,
    ) -> Result<Option<Rule>, RuntimeError>
    where
        Y: ClientApi<RuntimeError>,
    {
        let module_role_key = ModuleRoleKey::new(module, role_key);
        ...
        let rule = api.key_value_entry_get_typed::<RoleAssignmentRuleEntryPayload>(handle)?;

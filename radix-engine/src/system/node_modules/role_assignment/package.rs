...
use crate::system::node_modules::role_assignment::{LockRoleEvent, SetAndLockRoleEvent};
...
impl RoleAssignmentNativePackage {
    pub fn definition() -> PackageDefinition {
        ...
        functions.insert(
            ROLE_ASSINGMENT_LOCK_OWNER_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                ...
                export: ROLE_ASSINGMENT_LOCK_OWNER_IDENT.to_string(),
            },
        );
        functions.insert(
        ...
        functions.insert(
            ROLE_ASSINGMENT_GET_IDENT.to_string(),
            FunctionSchemaInit {
                receiver: Some(ReceiverInfo::normal_ref_mut()),
                input: TypeRef::Static(
                ...
                output: TypeRef::Static(
                    aggregator.add_child_type_and_descendents::<RoleAssingmentGetOutput>(),
                ),
                export: ROLE_ASSINGMENT_GET_IDENT.to_string(),
            },
        );
        ...
        let permission = match ident {
            ROLE_ASSIGNMENT_SET_IDENT => {
                ...
                ResolvedPermission::RoleList {
                    role_assignment_of: node_id.clone(),
                    role_list,
            ...
            ROLE_ASSINGMENT_GET_IDENT => ResolvedPermission::AllowAll,
            _ => {
        ...
        Y: ClientApi<RuntimeError>,
    {
        if roles.contains_key(&ObjectModuleId::RoleAssignment) {
            return Err(RuntimeError::ApplicationError(
                ApplicationError::AccessRulesError(RolesAssignmentError::UsedReservedSpace),
            ));
        }
        ...
        for (module, roles) in roles {
            for (role_key, role_def) in roles.data {
                if Self::is_reserved_role_key(&role_key) {
                    return Err(RuntimeError::ApplicationError(
                        ApplicationError::AccessRulesError(RolesAssignmentError::UsedReservedRole(

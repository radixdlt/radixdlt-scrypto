...
impl RoleAssignmentNativePackage {
    ...
    >(
        ...
        let owner_role = owner_role_substate.value.0.into_latest();
    ...
    > {
        ...
        let owner_role_field = match owner_role.updater {
            OwnerRoleUpdater::None => FieldValue::immutable(&RoleAssignmentOwnerFieldPayload::from_content_source(owner_role_substate)),
            OwnerRoleUpdater::Owner | OwnerRoleUpdater::Object => {
                FieldValue::new(&RoleAssignmentOwnerFieldPayload::from_content_source(owner_role_substate))
            }
        };
        ...
        for (module, roles) in roles {
            for (role_key, role_def) in roles.data {
                ...
                let value = role_def.map(|rule| scrypto_encode(&RoleAssignmentAccessRuleEntryPayload::from_content_source(rule)).unwrap());
        ...
        Ok((
            btreemap!(RoleAssignmentField::Owner.field_index() => owner_role_field),
            btreemap!(RoleAssignmentCollection::AccessRuleKeyValue.collection_index() => role_entries),
        ))
    }
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        let mut owner_role = api.field_read_typed::<RoleAssignmentOwnerFieldPayload>(handle)?.into_latest();
        owner_role.owner_role_entry.rule = rule.clone();
        api.field_write_typed(handle, &RoleAssignmentOwnerFieldPayload::from_content_source(owner_role))?;
        ...
        Y: ClientApi<RuntimeError>,
    {
        ...
        api.key_value_entry_set_typed(handle, RoleAssignmentAccessRuleEntryPayload::from_content_source(rule.clone()))?;

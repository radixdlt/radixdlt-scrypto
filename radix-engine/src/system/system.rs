...
#[derive(Debug, Clone, PartialEq, Eq, ScryptoSbor)]
pub struct TypedSubstateWrapper<V> {
    pub value: V,
    pub mutability: SubstateMutability,
}
    ...
    V: SystemCallbackObject,
{
    ...
    fn key_value_entry_remove_and_release_lock(
        ...
        let wrapper: TypedSubstateWrapper<Option<ScryptoValue>> =
    ...
    V: SystemCallbackObject,
{
    ...
    fn key_value_entry_get(
        ...
        self.api.kernel_read_substate(handle).map(|v| {
            let wrapper: TypedSubstateWrapper<Option<ScryptoValue>> = v.as_typed().unwrap();
    ...
    fn key_value_entry_freeze(&mut self, handle: KeyValueEntryHandle) -> Result<(), RuntimeError> {
        ...
        let mut wrapper: TypedSubstateWrapper<Option<ScryptoValue>> = v.as_typed().unwrap();

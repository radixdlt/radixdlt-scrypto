...
blueprint! {
    ...
    impl NftTest {
        ...
        pub fn update_and_get_nft() -> (Bucket, Bucket) {
            ...
            let mut data: Sandwich = resource_def.get_nft_data(&NftKey::from_u128(0));
            assert_eq!(data.available, false);

            data.available = true;
            resource_def.update_nft_data(&NftKey::from_u128(0), data, mint_badge.present());

            let data: Sandwich = resource_def.get_nft_data(&NftKey::from_u128(0));
        ...
        pub fn get_nft_ids_bucket() -> (Bucket, Bucket) {
            let mut bucket = Self::create_nft_fixed();
            let nft = bucket.take(1);
            assert_eq!(bucket.get_nft_ids(), Vec::from([NftKey::from_u128(2), NftKey::from_u128(3)]));
            assert_eq!(nft.get_nft_ids(), Vec::from([NftKey::from_u128(1)]));
            (bucket, nft)
        }
        ...
        pub fn get_nft_ids_vault() -> Bucket {
            let mut vault = Vault::with_bucket(Self::create_nft_fixed());
            let nft = vault.take(1);
            assert_eq!(vault.get_nft_ids(), Vec::from([NftKey::from_u128(2), NftKey::from_u128(3)]));
            assert_eq!(nft.get_nft_ids(), Vec::from([NftKey::from_u128(1)]));
            nft
        }

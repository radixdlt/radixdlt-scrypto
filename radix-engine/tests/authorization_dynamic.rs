...
fn dynamic_all_of_should_fail_on_nonexistent_resource() {
    test_dynamic_authlist(
        3,
        all_of!(resource_list!(SborPath::new().field("does_not_exist"))),
        &[0, 1, 2],
        false,
    );
}
...
fn dynamic_min_n_of_should_fail_if_not_signed_enough() {
    test_dynamic_authlist(3, min_n_of!(2, SborPath::new().field("auth")), &[0], false);
}
...
fn dynamic_all_of_should_allow_me_to_call_method() {
    test_dynamic_authlist(3, all_of!(SborPath::new().field("auth")), &[0, 1, 2], true);
}

#[test]
fn dynamic_all_of_should_fail_if_not_signed_enough() {
    test_dynamic_authlist(3, all_of!(SborPath::new().field("auth")), &[0, 1], false);
}

#[test]
fn dynamic_all_of_should_fail_if_path_does_not_exist() {
    test_dynamic_authlist(3, all_of!(SborPath::new().field("does_not_exist")), &[0, 1], false);
}

#[test]
fn dynamic_any_of_should_allow_me_to_call_method() {
    test_dynamic_authlist(3, any_of!(SborPath::new().field("auth")), &[1], true);
}

#[test]
fn dynamic_any_of_should_fail_if_path_does_not_exist() {
    test_dynamic_authlist(3, any_of!(SborPath::new().field("does_not_exist")), &[0, 1], false);
}

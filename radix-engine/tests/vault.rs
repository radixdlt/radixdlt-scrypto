...
fn non_existent_vault_in_component_creation_should_fail() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "NonExistentVault",
            invocation!(create_component_with_non_existent_vault()),
        )
...
fn non_existent_vault_in_committed_component_should_fail() {
    ...
    let package = executor
        ...
        .unwrap();
    let transaction = TransactionBuilder::new()
        .call_function(package, "NonExistentVault", invocation!(new()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_method(component_address, invocation!(create_non_existent_vault()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn non_existent_vault_in_lazy_map_creation_should_fail() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "NonExistentVault",
            invocation!(create_lazy_map_with_non_existent_vault()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn non_existent_vault_in_committed_lazy_map_should_fail() {
    ...
    let transaction = TransactionBuilder::new()
        .call_method(
            component_address,
            invocation!(create_non_existent_vault_in_lazy_map()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn dangling_vault_should_fail() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(dangling_vault()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_into_map() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(new_vault_into_map()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn invalid_double_ownership_of_vault() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "VaultTest",
            invocation!(invalid_double_ownership_of_vault()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_into_map_and_referencing_before_storing() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(new_vault_into_map_then_get()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn cannot_overwrite_vault_in_map() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_method(component_address, invocation!(overwrite_vault_in_map()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_into_vector() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(new_vault_into_vector()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn can_push_vault_into_vector() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_method(component_address, invocation!(push_vault_into_vector()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_with_take() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(new_vault_with_take()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_with_take_non_fungible() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "VaultTest",
            invocation!(new_vault_with_take_non_fungible()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_with_get_nonfungible_ids() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "VaultTest",
            invocation!(new_vault_with_get_non_fungible_ids()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_with_get_amount() {
    ...
    // Act
    let transaction = TransactionBuilder::new()
        .call_function(package, "VaultTest", invocation!(new_vault_with_get_amount()))
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();
...
fn create_mutable_vault_with_get_resource_manager() {
    ...
    let transaction = TransactionBuilder::new()
        .call_function(
            package,
            "VaultTest",
            invocation!(new_vault_with_get_resource_manager()),
        )
        .build(executor.get_nonce([]))
        .sign([]);
    let receipt = executor.validate_and_execute(&transaction).unwrap();

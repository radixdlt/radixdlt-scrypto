...
fn can_create_clone_and_drop_vault_proof_by_ids() {
    ...
    let proof_ids = BTreeSet::from([NonFungibleId::NumberU32(2)]);
    let manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        .lock_fee(FAUCET_COMPONENT, 10.into())
        .call_method(
            component_address,
        ...
        .take_from_worktop_by_ids(
            ...
            |builder, bucket_id| {
                builder.call_method(
                    ...
                    args!(
                        Bucket(bucket_id),
                        BTreeSet::from([NonFungibleId::NumberU32(1), NonFungibleId::NumberU32(2),])
                    ),
                )
            },
        )
...
fn can_create_auth_zone_proof_by_amount_from_non_fungibles() {
    ...
    let manifest = ManifestBuilder::new(&NetworkDefinition::simulator())
        ...
        .create_proof_from_account_by_ids(
            account,
            &BTreeSet::from([NonFungibleId::NumberU32(3)]),
            resource_address,
        )

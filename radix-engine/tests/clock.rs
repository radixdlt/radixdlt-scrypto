...
use radix_engine_interface::wasm::{ClockInvocation, NativeFnInvocation, NativeFunctionInvocation, NativeMethodInvocation};
use scrypto_unit::*;
...
fn a_new_clock_instance_can_be_created_by_the_system() {
    ...
    let instructions = vec![Instruction::System(NativeFnInvocation::Method(
        NativeMethodInvocation::Clock(ClockInvocation::Create(ClockCreateInvocation {})),
    ))];
    let blobs = vec![];

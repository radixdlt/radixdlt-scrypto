...
pub fn validate_transaction_from_slice(
    transaction: &[u8],
    current_epoch: u64,
) -> Result<ValidatedTransaction, TransactionValidationError> {
    let transaction: NotarizedTransaction =
        scrypto_decode(transaction).map_err(TransactionValidationError::DeserializationError)?;

    validate_transaction(transaction, current_epoch)
}
...
pub fn validate_transaction(
    ...
    validate_header(&transaction, current_epoch)
    ...
    for inst in &transaction.signed_intent.intent.manifest.instructions {
        match inst.clone() {
            ...
            Instruction::CallFunction {
                ...
                validate_call_data(&call_data, &mut id_validator)

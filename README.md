...
1. Install Rust - this requires Rust 1.60+ (if rust is already installed, upgrade with `rustup update`)
   ...
   * macOS:
       * Make sure the xcode command line tools is installed by running: `xcode-select --install`
       * Install Rust compiler
       ```
       curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
       ```
   * Linux:
       * Make sure a C++ compiler and LLVM is installed (`sudo apt install clang build-essential llvm` with Ubuntu)
       * Install Rust compiler
       ```
       curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
       ```

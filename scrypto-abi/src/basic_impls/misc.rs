...
impl<'a, X: CustomTypeId, B: ?Sized + 'a + ToOwned + Schema<X>> Schema<X> for Cow<'a, B>
{
    ...
    fn get_local_type_data() -> Option<LocalTypeData<TypeRef>> { <B>::get_local_type_data() }

    fn add_all_dependencies(aggregator: &mut SchemaAggregator<X>) { <B>::add_all_dependencies(aggregator) }
}

...
impl ResourceManager {
    ...
    pub fn lock_mintable(&self) {
        let access_rules = self.0.access_rules();
        access_rules.set_authority_mutability(MINT_AUTHORITY, RoleList::none());
    }

    pub fn lock_burnable(&self) {
        let access_rules = self.0.access_rules();
        access_rules.set_authority_mutability(BURN_AUTHORITY, RoleList::none());
    }

    pub fn lock_updateable_metadata(&self) {
        let access_rules = self.0.access_rules();
        access_rules.set_authority_mutability(UPDATE_METADATA_AUTHORITY, RoleList::none());
    }
    ...
    pub fn lock_depositable(&self) {
        let access_rules = self.0.access_rules();
        access_rules.set_authority_mutability(DEPOSIT_AUTHORITY, RoleList::none());
    }

    pub fn lock_recallable(&self) {
        let access_rules = self.0.access_rules();
        access_rules.set_authority_mutability(RECALL_AUTHORITY, RoleList::none());
    }
}

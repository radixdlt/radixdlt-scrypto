...
impl ResourceBuilder {
    ...
    pub fn new_non_fungible<Y: IntoNonFungibleLocalId>() -> NonFungibleResourceBuilder<Y> {
...
impl<Y: IntoManualNonFungibleLocalId> NonFungibleResourceBuilder<Y> {
    /// Creates resource with the given initial supply.
    ///
    /// # Example
    /// ```ignore
...
impl<Y: IntoManualNonFungibleLocalId> NonFungibleResourceWithAuthBuilder<Y> {
    pub fn initial_supply<T, V>(self, entries: T) -> Bucket
    where
        T: IntoIterator<Item = (Y, V)>,
        V: NonFungibleData,

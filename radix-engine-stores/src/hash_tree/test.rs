...
fn hash_of_next_version_differs_when_value_changed() {
    ...
    let hash_v1 = put_at_next_version(
        &mut store,
        None,
        vec![change(1, SysModuleId::Tuple, 2, Some(30))],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, SysModuleId::Tuple, 2, Some(70))],
    );
...
fn hash_of_next_version_same_when_write_repeated() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        None,
        vec![
            change(4, SysModuleId::Tuple, 6, Some(30)),
            change(3, SysModuleId::Tuple, 9, Some(40)),
        ],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(4, SysModuleId::Tuple, 6, Some(30))],
    );
...
fn hash_of_next_version_same_when_write_empty() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        None,
        vec![
            change(1, SysModuleId::Tuple, 2, Some(30)),
            change(3, SysModuleId::Tuple, 1, Some(40)),
        ],
    );
...
fn hash_of_next_version_differs_when_substate_added() {
    ...
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, SysModuleId::Tuple, 8, Some(30))],
    );
    assert_ne!(hash_v1, hash_v2);
}
...
fn hash_of_next_version_differs_when_substate_removed() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        vec![
            change(1, SysModuleId::Tuple, 2, Some(30)),
            change(4, SysModuleId::Tuple, 3, Some(20)),
        ],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, SysModuleId::Tuple, 2, None)],
    );
    assert_ne!(hash_v1, hash_v2);
}
...
fn hash_returns_to_same_when_previous_state_restored() {
    ...
    put_at_next_version(
        ...
        Some(1),
        vec![
            change(1, SysModuleId::Tuple, 2, Some(90)),
            change(3, SysModuleId::Tuple, 1, None),
            change(1, SysModuleId::Tuple, 5, Some(10)),
        ],
    );
    let hash_v3 = put_at_next_version(
        ...
        vec![
            change(1, SysModuleId::Tuple, 2, Some(30)),
            change(3, SysModuleId::Tuple, 1, Some(40)),
            change(1, SysModuleId::Tuple, 5, None),
        ],
    );
...
fn hash_differs_when_states_only_differ_by_node_id() {
    ...
    let hash_1 = put_at_next_version(
        &mut store_1,
        None,
        vec![change(1, SysModuleId::Tuple, 3, Some(30))],
    );
    let mut store_2 = TypedInMemoryTreeStore::new();
    let hash_2 = put_at_next_version(
        &mut store_2,
        None,
        vec![change(2, SysModuleId::Tuple, 3, Some(30))],
    );
...
fn records_stale_tree_node_keys() {
    ...
    put_at_next_version(
        &mut store,
        Some(1),
        vec![change(3, SysModuleId::Tuple, 9, Some(70))],
    );
    put_at_next_version(
        &mut store,
        Some(2),
        vec![change(3, SysModuleId::Tuple, 9, Some(80))],
    );
...
fn sbor_decodes_what_was_encoded() {
    let nodes = vec![
        ...
        TreeNode::Leaf(TreeLeafNode {
            ...
            payload: substate_id(13, SysModuleId::Tuple, 37),
            value_hash: Hash([7; 32]),
        }),
        TreeNode::Null,
    ];
...
fn serialized_keys_are_strictly_increasing() {
    ...
    put_at_next_version(
        &mut store,
        None,
        vec![change(3, SysModuleId::Tuple, 4, Some(90))],
    );
    let previous_key = store.memory.keys().collect_vec()[0].clone();
    put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, SysModuleId::Tuple, 2, Some(80))],
    );
    let next_key = store
        .memory
        .keys()

...
use radix_engine_interface::types::{
    ...
    METADATA_BASE_MODULE, OBJECT_BASE_MODULE,
};
...
fn hash_of_next_version_differs_when_value_changed() {
    ...
    let hash_v1 = put_at_next_version(
        &mut store,
        None,
        vec![change(1, OBJECT_BASE_MODULE, 2, Some(30))],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, OBJECT_BASE_MODULE, 2, Some(70))],
    );
...
fn hash_of_next_version_same_when_write_repeated() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        None,
        vec![
            change(4, OBJECT_BASE_MODULE, 6, Some(30)),
            change(3, OBJECT_BASE_MODULE, 9, Some(40)),
        ],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(4, OBJECT_BASE_MODULE, 6, Some(30))],
    );
...
fn hash_of_next_version_same_when_write_empty() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        None,
        vec![
            change(1, OBJECT_BASE_MODULE, 2, Some(30)),
            change(3, OBJECT_BASE_MODULE, 1, Some(40)),
        ],
    );
...
fn hash_of_next_version_differs_when_substate_added() {
    ...
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, OBJECT_BASE_MODULE, 8, Some(30))],
    );
    assert_ne!(hash_v1, hash_v2);
}
...
fn hash_of_next_version_differs_when_substate_removed() {
    ...
    let hash_v1 = put_at_next_version(
        ...
        vec![
            change(1, OBJECT_BASE_MODULE, 2, Some(30)),
            change(4, OBJECT_BASE_MODULE, 3, Some(20)),
        ],
    );
    let hash_v2 = put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, OBJECT_BASE_MODULE, 2, None)],
    );
    assert_ne!(hash_v1, hash_v2);
}
...
fn hash_returns_to_same_when_previous_state_restored() {
    ...
    put_at_next_version(
        ...
        Some(1),
        vec![
            change(1, OBJECT_BASE_MODULE, 2, Some(90)),
            change(3, OBJECT_BASE_MODULE, 1, None),
            change(1, OBJECT_BASE_MODULE, 5, Some(10)),
        ],
    );
    let hash_v3 = put_at_next_version(
        ...
        vec![
            change(1, OBJECT_BASE_MODULE, 2, Some(30)),
            change(3, OBJECT_BASE_MODULE, 1, Some(40)),
            change(1, OBJECT_BASE_MODULE, 5, None),
        ],
    );
...
fn hash_differs_when_states_only_differ_by_node_id() {
    ...
    let hash_2 = put_at_next_version(
        &mut store_2,
        None,
        vec![change(2, OBJECT_BASE_MODULE, 3, Some(30))],
    );
...
fn records_stale_tree_node_keys() {
    ...
    put_at_next_version(
        &mut store,
        Some(1),
        vec![change(3, OBJECT_BASE_MODULE, 9, Some(70))],
    );
    put_at_next_version(
        &mut store,
        Some(2),
        vec![change(3, OBJECT_BASE_MODULE, 9, Some(80))],
    );
...
fn serialized_keys_are_strictly_increasing() {
    ...
    put_at_next_version(
        &mut store,
        None,
        vec![change(3, OBJECT_BASE_MODULE, 4, Some(90))],
    );
    let previous_key = store.memory.keys().collect_vec()[0].clone();
    put_at_next_version(
        &mut store,
        Some(1),
        vec![change(1, OBJECT_BASE_MODULE, 2, Some(80))],
    );
    let next_key = store
        .memory
        .keys()

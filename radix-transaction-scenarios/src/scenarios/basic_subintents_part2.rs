...
pub struct BasicSubintentsPart2ScenarioConfig {
    pub parent_account_key: PrivateKey,
    pub verified_parent_key: PrivateKey,
    pub child_account_key: PrivateKey,
}

impl Default for BasicSubintentsPart2ScenarioConfig {
    fn default() -> Self {
        Self {
            parent_account_key: new_ed25519_private_key(3).into(),
            verified_parent_key: new_ed25519_private_key(542).into(),
...
#[derive(Default)]
pub struct BasicSubintentsPart2ScenarioState {
...
pub struct BasicSubintentsPart2ScenarioCreator;

impl ScenarioCreator for BasicSubintentsPart2ScenarioCreator {
    type Config = BasicSubintentsPart2ScenarioConfig;
    type State = BasicSubintentsPart2ScenarioState;
    type Instance = Scenario<Self::Config, Self::State>;

    const METADATA: ScenarioMetadata = ScenarioMetadata {
        logical_name: "basic_subintents_part2",
        // This scenario replaces / fixes `basic_subintents`
        protocol_min_requirement: ProtocolVersion::CuttlefishPart2,
        protocol_max_requirement: ProtocolVersion::LATEST,
        testnet_run_at: Some(ProtocolVersion::CuttlefishPart2),
        ...
        ScenarioBuilder::new(core, Self::METADATA, config, start_state)
            ...
            .successful_transaction(|core, config, state| {
                    ...
                    .manifest_builder_with_lookup(|builder, lookup| {
                        builder
                            .verify_parent(rule!(require(
                                config.verified_parent_key.public_key().signature_proof()
                            )))
                            .withdraw_from_account(
                                state.child_account.unwrap(),
                                state.child_token.unwrap(),
                ...
                let complex_subintent = core
                    ...
                    .sign(&config.verified_parent_key)
                    .complete(core);
            ...
            .finalize(|core, config, state| {
                Ok(ScenarioOutput {
                    interesting_addresses: DescribedAddresses::new()
                        ...
                        .add(
                            "verified_parent_signature_proof",
                            config.verified_parent_key.public_key().signature_proof(),
                        ),
                })
            })
